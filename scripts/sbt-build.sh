...
sbt -J-XX:ReservedCodeCacheSize=128m ++$TRAVIS_SCALA_VERSION ";izanami-server/assembly;izanami-server/dist"
sbt -J-XX:ReservedCodeCacheSize=128m ++$TRAVIS_SCALA_VERSION ";izanami-server/docker:publish"
...
if [-z "$TRAVIS_TAG"];
then
    echo 'Not a tag, nothing to to do'
else
echo "Tag ${TRAVIS_TAG}, Publishing client"
    sbt -J-XX:ReservedCodeCacheSize=128m ++$TRAVIS_SCALA_VERSION "project jvm" "publish"
    echo "Uploading izanami.jar"
