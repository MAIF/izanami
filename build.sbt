...
lazy val excludeLegacyClientDep = Seq(
  ExclusionRule(organization = "com.fasterxml.jackson.core"),
  ExclusionRule(organization = "com.fasterxml.jackson.datatype"),
  ExclusionRule(organization = "com.fasterxml.jackson.dataformat")
)
...
libraryDependencies += "org.flywaydb" % "flyway-database-postgresql" % "12.6.1"
...
libraryDependencies += "org.mozilla" % "rhino" % "1.7.15.1"
...
libraryDependencies += "com.github.jknack" % "handlebars" % "4.5.1"
libraryDependencies += "com.github.jknack" % "handlebars-jackson" % "4.5.1" excludeAll (excludesJackson: _*)
...
libraryDependencies += "org.apache.pekko" %% "pekko-protobuf-v3" % "1.6.0"
libraryDependencies += "org.apache.pekko" %% "pekko-stream" % "1.6.0"
...
libraryDependencies += "com.fasterxml.jackson.module" %% "jackson-module-scala" % "2.13.5" % Test
...
libraryDependencies += "org.apache.pekko" %% "pekko-connectors-sse" % "1.3.0" % Test
...
assembly / assemblyMergeStrategy := {
  ...
  case referenceOverrides if referenceOverrides.contains("module-info.class") =>
    MergeStrategy.discard
  case referenceOverrides if referenceOverrides.contains("mailcap.default") =>
    MergeStrategy.first
  case PathList("javax", xs @ _*) => MergeStrategy.first
  case PathList("javax", xs @ _*) => MergeStrategy.first
  case referenceOverrides if referenceOverrides.startsWith("scala/") =>
    MergeStrategy.first
  case referenceOverrides if referenceOverrides.startsWith("play/") =>
    MergeStrategy.first
  case referenceOverrides
      if referenceOverrides.contains("library.properties") =>
    MergeStrategy.first
  case referenceOverrides if referenceOverrides.contains("mimetypes.default") =>
    MergeStrategy.first
  case PathList("META-INF", x, xs @ _*) if x.toLowerCase == "services" =>
