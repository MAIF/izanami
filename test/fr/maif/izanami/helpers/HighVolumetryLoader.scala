...
object HighVolumetryLoader {
  def main(args: Array[String]): Unit = {
    ...
    await(
      Source
        .fromIterator(() => projectNames.iterator)
        .mapAsync(10)(p =>
          Future {
    ...
    await(
      Source
        .fromIterator(() => projectFeatureTuples.iterator)
        .mapAsync(200) { case (project, name) =>
          Future {
            println(s"Creating feature ${name} for project ${project}")
            situation.createFeature(name, project, "load")
          }(ExecutionContext.global)
        }
        .run()(mat)
    )(Timeout.durationToTimeout(1200.seconds))
    System.exit(0)
  }
}
