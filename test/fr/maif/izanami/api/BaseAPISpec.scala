...
class BaseAPISpec extends PlaySpec with BeforeAndAfterEach with BeforeAndAfterAll with DefaultAwaitTimeout with IzanamiServerTest {
...
object BaseAPISpec extends DefaultAwaitTimeout {
  ...
  def testFeature(
    ...
    val response = await(
      ws.url(s"""${ADMIN_BASE_URL}/tenants/$tenant/test?date=${DateTimeFormatter.ISO_INSTANT.format(date)}${if (user != null)
        s"&user=${user}"
      else ""}""")
        .withCookies(cookies: _*)
        .post(Json.parse(s"""{
  ...
  case class TestSituation(
    ...
    def createLegacyFeature(
      ...
      val response = await(ws.url(s"${ADMIN_BASE_URL}/tenants/${tenant}/projects/${project}/features")
        .withCookies(cookies: _*)
        .post(Json.parse(
          s"""{
             |"id": "$id",
    ...
    def readFeatureAsLegacy(pattern: String, clientId: String, clientSecret: String, body: Option[JsValue] = None): RequestResult = {
      val response = await(body.map(json => {
        ws.url(s"${BASE_URL}/features/${pattern}/check").withHttpHeaders(("Izanami-Client-Id" -> clientId), ("Izanami-Client-Secret" -> clientSecret)).post(json)
      }).getOrElse(ws.url(s"${BASE_URL}/features/${pattern}/check").withHttpHeaders(("Izanami-Client-Id" -> clientId), ("Izanami-Client-Secret" -> clientSecret)).get()))

      RequestResult(Try{response.json}, status=response.status)
    }

    def readFeaturesAsLegacy(pattern: String, clientId: String, clientSecret: String, active: Boolean = false, page: Int = 1, pageSize: Int = 100): RequestResult = {
    ...
    def createFeature(
      ...
      BaseAPISpec.this.createFeature(name, project, tenant, enabled, tags, metadata, conditions, wasmConfig, id, cookies)
    }
    ...
    def testExistingFeature(
      ...
      val response = await(
        ws.url(s"""${ADMIN_BASE_URL}/tenants/${tenant}/features/${featureId}/test${if(context.nonEmpty) s"/${context}" else "" }?date=${DateTimeFormatter.ISO_INSTANT
    ...
    def checkFeature(id: String, key: String, user: String = null, context: String = null, conditions: Boolean=false): RequestResult = {
    ...
    def deleteImportResult(tenant: String, id: String): RequestResult = {
      val response = await(ws.url(s"${ADMIN_BASE_URL}/tenants/${tenant}/_import/$id")
        .withCookies(cookies: _*)
        .delete()
      )
    ...
    def importAndWaitTermination(
      ...
      val response = importV1Data(tenant, timezone, conflictStrategy, deduceProject, project, projectPartSize, features, users, keys, scripts, inlineScript)
    ...
    def importV1Data(
      ...
      val response = await(ws.url(s"${ADMIN_BASE_URL}/tenants/${tenant}/_import?timezone=${timezone}&conflict=${conflictStrategy}&deduceProject=${deduceProject}${Option(project).map(p => s"&project=${p}").getOrElse("")}${projectPartSize.map(p => s"&projectPartSize=${p}").getOrElse("")}&inlineScript=${inlineScript}").withCookies(cookies: _*).post(
        Source(
          Seq(
        ...
        .flatMap(_ => {
          ...
          wasmScripts.foldLeft(Future.successful(())) {
            case (future, TestWasmScript(name, fileContent)) => {
              future.flatMap(_ => wasmManagerClient.createScript(name, fileContent, local=false))
                .map(ids => {
