...
class ApplicationKeysAPISpec extends BaseAPISpec {
  "API key POST endpoint" should {
    "allow to create API key" in {
      ...
      (response.json.get \ "clientSecret").get.as[String] must startWith("my-tenant")
  ...
  "API Key GET endpoint" should {
    "return all API keys without secrets for given tenant" in {
      val situation = TestSituationBuilder()
        .loggedInWithAdminRights()
        .withTenants(
          TestTenant("my-tenant")
      ...
      (result.json.get \\ "clientSecret").map(v => v.asOpt[String]).filter(_.isDefined) mustBe empty
