...
class LoginAPISpec extends BaseAPISpec {
  ...
  "Openidconnect endpoints" should {
    ...
    "prevent oidc user right update if role-right-mode is supervised" in {
      ...
      var updateResponse = situation.updateUserRights(name = "Sam Tailor", admin = true, rights = TestRights())
      updateResponse.status mustBe BAD_REQUEST
      updateResponse = situation.updateUserRights(
        name = "Sam Tailor",
        admin = false,
        rights = TestRights(tenants = Map("foo" -> TestTenantRight("foo", level = "Write")))
      )
      updateResponse.status mustBe BAD_REQUEST
    }
    ...
    "allow oidc user right update if role-right-mode is initial" in {
      var situation: TestSituation = TestSituationBuilder()
        .withTenantNames("foo")
        .withCustomConfiguration(baseOIDCConfiguration + ("app.openid.role-right-mode" -> "initial"))
        .build()
      val unauthorizedResponse     = situation.fetchTenants()
      unauthorizedResponse.status mustBe UNAUTHORIZED
      situation = situation.logAsOIDCUser("User1")
    ...
    "assign default roles if user don't have any" in {
      var situation: TestSituation = TestSituationBuilder()
        .withTenants(TestTenant("test").withProjectNames("proj").withApiKeyNames("apikey"))
      ...
      (rights \ "rights" \ "tenants" \ "test" \ "level").as[String] mustEqual "Read"
      (rights \ "rights" \ "tenants" \ "test" \ "defaultProjectRight").as[String] mustEqual "Update"
      (rights \ "rights" \ "tenants" \ "test" \ "defaultKeyRight").as[String] mustEqual "Read"
      (rights \ "rights" \ "tenants" \ "test" \ "defaultWebhookRight").as[String] mustEqual "Write"
      (rights \ "rights" \ "tenants" \ "test" \ "projects" \ "proj" \ "level").as[String] mustEqual "Admin"
      (rights \ "rights" \ "tenants" \ "test" \ "keys" \ "apikey" \ "level").as[String] mustEqual "Admin"
    }
    ...
    "merge rights to keep only highers" in {
      ...
      val rights      = rightResult.json.get
      (rights \ "admin").as[Boolean] mustEqual true
      (rights \ "rights" \ "tenants" \ "test" \ "level").as[String] mustEqual "Write"
      (rights \ "rights" \ "tenants" \ "test" \ "defaultProjectRight").as[String] mustEqual "Update"
      (rights \ "rights" \ "tenants" \ "test" \ "defaultKeyRight").as[String] mustEqual "Read"
      (rights \ "rights" \ "tenants" \ "test" \ "defaultWebhookRight").as[String] mustEqual "Write"
      (rights \ "rights" \ "tenants" \ "test" \ "projects" \ "proj" \ "level").as[String] mustEqual "Write"
      (rights \ "rights" \ "tenants" \ "test" \ "keys" \ "apikey" \ "level").as[String] mustEqual "Admin"
    }
    ...
    "Apply right changes if role association change" in {
      ...
      (rights \ "rights" \ "tenants" \ "test" \ "level").as[String] mustEqual "Write"
      (rights \ "rights" \ "tenants" \ "test" \ "defaultProjectRight").as[String] mustEqual "Write"
      (rights \ "rights" \ "tenants" \ "test" \ "defaultKeyRight").as[String] mustEqual "Write"
      (rights \ "rights" \ "tenants" \ "test" \ "defaultWebhookRight").as[String] mustEqual "Write"
      (rights \ "rights" \ "tenants" \ "test" \ "projects" \ "proj" \ "level").as[String] mustEqual "Write"
      (rights \ "rights" \ "tenants" \ "test" \ "keys" \ "apikey" \ "level").as[String] mustEqual "Write"
    ...
    "Keep rights in memory if env variable are set then unset" in {
      ...
      (rights \ "admin").as[Boolean] mustEqual true
      (rights \ "rights" \ "tenants" \ "test" \ "level").as[String] mustEqual "Read"
      (rights \ "rights" \ "tenants" \ "test" \ "defaultProjectRight").as[String] mustEqual "Read"
      (rights \ "rights" \ "tenants" \ "test" \ "defaultKeyRight").as[String] mustEqual "Read"
      (rights \ "rights" \ "tenants" \ "test" \ "defaultWebhookRight").as[String] mustEqual "Read"
      (rights \ "rights" \ "tenants" \ "test" \ "projects" \ "proj" \ "level").as[String] mustEqual "Read"
      (rights \ "rights" \ "tenants" \ "test" \ "keys" \ "apikey" \ "level").as[String] mustEqual "Read"
    }
    ...
    "Prevent right for role update if they are set from env" in {
      ...
      val result = situation.updateConfigurationWithCallback(conf => {
        val jsonTransformer =
          (__ \ "oidcConfiguration" \ "userRightsByRoles" \ "" \ "admin").json.update(__.read[JsBoolean].map { _ =>
            JsFalse
          })
