...
class TagAPISpec extends BaseAPISpec {
  ...
  "Tag POST endpoint" should {
    "prevent tag creation if tag name or description is too long" in {
      ...
      val situation = TestSituationBuilder()
        ...
        .build()
      var tagResponse =
        situation.createTag("abcdefghij" * 21, tenantName, "My description")
      tagResponse.status mustBe BAD_REQUEST

      tagResponse =
        situation.createTag("abcdefghij", tenantName, "abcdefghij" * 51)
      tagResponse.status mustBe BAD_REQUEST
    }
    ...
    "allow tag creation" in {
      ...
      val situation = TestSituationBuilder()
        ...
        .build()
      val tagResponse =
        situation.createTag("my-tag", tenantName, "My description")
  ...
  "Tag GET endpoint" should {
      ...
      val situation = TestSituationBuilder()
        ...
        .withTenants(
          TestTenant(tenantName)
            .withTags(TestTag(name = tagName, description = "My description"))
        )
        .build();
    ...
    "retrieve existing tags" in {
      ...
      fetchResponse.status mustBe OK
      (fetchResponse.json.get \\ "name").map(v =>
        v.as[String]
  ...
  "Tag PUT endpoint" should {
    "prevent tag update if new name or description is too long" in {
      ...
      var updateResponse = situation.updateTag(
        tenantName,
        TestTag("abcdefghij" * 21, "my-description"),
        "Tag"
      )
      updateResponse.status mustBe BAD_REQUEST

      updateResponse = situation.updateTag(
        tenantName,
        TestTag("supertag", "abcdefghij" * 51),
        "Tag"
      )
      updateResponse.status mustBe BAD_REQUEST
    }
    ...
    "allow tag update" in {
      ...
      val situation = TestSituationBuilder()
        ...
        .loggedInWithAdminRights()
        .build()
      val updateResponse = situation.updateTag(
        tenantName,
        TestTag("my-tag", "my-description"),
        "Tag"
      )
