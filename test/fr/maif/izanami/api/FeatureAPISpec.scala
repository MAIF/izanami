...
import fr.maif.izanami.api.BaseAPISpec.TestFeaturePatch
...
import fr.maif.izanami.api.BaseAPISpec.deleteFeatureWithToken
import fr.maif.izanami.api.BaseAPISpec.disabledFeatureBase64
import fr.maif.izanami.api.BaseAPISpec.enabledFeatureBase64
import fr.maif.izanami.api.BaseAPISpec.patchFeatureWithToken
import fr.maif.izanami.api.BaseAPISpec.updateFeatureWithToken
...
class FeatureAPISpec extends BaseAPISpec {
  "Feature PATCH endpoint" should {
    ...
    "allow to modify feature enabling if user has default update right on projects" in {
      val situation = TestSituationBuilder()
        ...
        .withUsers(
          TestUser("testu")
            .withTenantReadRight("tenant")
            .withDefaultUpdateProjectRight("tenant")
        )
        .loggedAs("testu")
        .build()
      ...
      ((project \ "features")
        .as[Seq[JsObject]]
        .find(obj => (obj \ "name").as[String] == "f1")
        .get \ "enabled")
    ...
    "allow feature delete if user has default write right for projects" in {
      val situation = TestSituationBuilder()
        .withTenants(
          TestTenant("tenant")
            .withProjects(
              TestProject("project").withFeatureNames("F1", "F2", "F3")
            )
        )
        .withUsers(
          TestUser("testu")
            .withTenantReadRight("tenant")
            .withDefaultWriteProjectRight("tenant")
        )
        .loggedAs("testu")
        .build()
    ...
    "prevent feature delete if user has update right on feature" in {
      val situation = TestSituationBuilder()
        ...
        .withUsers(
          TestUser("testu")
            .withTenantReadRight("tenant")
            .withProjectReadUpdateRight(project = "project", tenant = "tenant")
        )
        .loggedAs("testu")
        .build()
    ...
    "allow to modify multiple features enabling" in {
      ...
      val response = situation.patchFeatures(
        ...
        Seq(
          ...
          TestFeaturePatch(
            op = "replace",
            path =
              s"/${situation.findFeatureId("tenant", "project2", "f4").get}/enabled",
            value = JsBoolean(true)
          ),
          TestFeaturePatch(
            op = "replace",
            path =
              s"/${situation.findFeatureId("tenant", "project2", "f5").get}/enabled",
            value = JsBoolean(false)
          )
        )
      )
      ...
      ((project \ "features")
        .as[Seq[JsObject]]
        .find(obj => (obj \ "name").as[String] == "f2")
        .get \ "enabled")
        .as[Boolean] mustBe false
      ((project \ "features")
        .as[Seq[JsObject]]
        .find(obj => (obj \ "name").as[String] == "f3")
        .get \ "enabled")
        .as[Boolean] mustBe false
      ...
      ((project2 \ "features")
        .as[Seq[JsObject]]
        .find(obj => (obj \ "name").as[String] == "f4")
        .get \ "enabled")
        .as[Boolean] mustBe true
      ((project2 \ "features")
        .as[Seq[JsObject]]
        .find(obj => (obj \ "name").as[String] == "f5")
        .get \ "enabled")
        .as[Boolean] mustBe false
    }
      ...
      val response = situation.patchFeatures(
        ...
        patches = Seq(
          ...
          TestFeaturePatch(
            op = "replace",
            path =
              s"/${situation.findFeatureId("tenant", "project", "F2").get}/project",
            value = JsString("new-project")
          )
        )
      )
      ...
      val response = situation.patchFeatures(
        ...
        Seq(
          TestFeaturePatch(
            op = "replace",
            path =
              s"/${situation.findFeatureId("tenant", "project", "F1").get}/tags",
            value = JsArray(
              Seq(JsString("t1"), JsString("abcdefghij" * 21), JsString("t4"))
            )
          ),
          TestFeaturePatch(
            op = "replace",
            path =
              s"/${situation.findFeatureId("tenant", "project", "F2").get}/tags",
  ...
  "Feature POST endpoint" should {
    ...
    "prevent non legacy feature creation if force-legacy param is set" in {
      val testSituation = TestSituationBuilder()
        ...
        .withCustomConfiguration(Map("app.feature.force-legacy" -> "true"))
        .loggedInWithAdminRights()
        .build()

      val result =
        testSituation.createLegacyFeature(
          id = "myfeature",
          name = "myfeature",
          project = "bar",
          tenant = "foo"
      ...
      modernResult.status mustBe BAD_REQUEST
    }
      ...
      val testSituation = TestSituationBuilder()
        ...
        .withUsers(
          TestUser("testu")
            ...
            .withDefaultWriteProjectRight("foo")
        )
        .loggedAs("testu")
        .build()

      val result = testSituation.createFeature(
        "my-feature",
        project = "bar",
        tenant = "foo"
      )
    ...
    "prevent to creating a feature if user has update role on project" in {
      val testSituation = TestSituationBuilder()
        ...
        .withUsers(
          TestUser("updateRight")
            .withTenantReadRight("foo")
            .withProjectReadUpdateRight(project = "bar", tenant = "foo")
        )
    ...
    "reject string or number features if conditions don't have values" in {
      ...
      var result = testSituation.createFeature(
        ...
        value = "1",
        conditions =
          Set(TestCondition(rule = TestUserListRule(users = Set("me"))))
      )
      result.status mustBe BAD_REQUEST
    ...
    "allow to create feature with number result type" in {
      ...
      result.status mustBe CREATED
      val featureJson =
        testSituation.fetchProject("foo", "bar").json.get \ "features" \ 0
    ...
    "allow 'RELEASE_DATE' feature creation" in {
      ...
      val response = testSituation.createFeature(
        ...
        conditions = Set(
          TestCondition(period =
            TestDateTimePeriod(begin =
              LocalDateTime.of(1992, 8, 18, 10, 12, 13)
            )
          )
        )
      )
      ...
      response.status mustBe CREATED
      (response.json.get \ "conditions" \\ "begin").head
        .as[String] mustEqual "1992-08-18T10:12:13Z"
    }
    ...
    "allow period feature creation" in {
      ...
      (json.get \ "conditions" \\ "begin").head
        .as[String] mustEqual "2022-01-01T12:23:43Z"
      (json.get \ "conditions" \\ "end").head
        .as[String] mustEqual "2023-01-01T12:23:43Z"
    }
    ...
    "allow metadata in feature creation" in {
      ...
      response.status mustBe CREATED
      (response.json.get \ "metadata" \ "env").get
        .as[String] mustEqual "test-env"
    }
    ...
    "allow to create feature with tags" in {
      ...
      (featureResponse.json.get \ "tags")
        .as[Seq[JsString]]
        .map(v => v.value) must contain theSameElementsAs Seq(
        "my-tag"
      )
    }
  ...
  "Feature PUT endpoint" should {
    ...
    "allow to update feature activation if user has default update right on projects" in {
      val situation = TestSituationBuilder()
        ...
        .withUsers(
          TestUser("updateUser")
            .withTenantReadRight("tenant")
            .withDefaultUpdateProjectRight(tenant = "tenant")
        )
        .loggedAs("updateUser")
        .build();
    ...
    "prevent feature update if field are too long" in {
      ...
      var updateResponse =
        situation.updateFeature("tenant", featureId, newFeature)
      ...
      newFeature = jsonFeature ++ Json.obj(
        "resultType" -> "string",
        "value" -> "abcdefghijklmnopqrstuvwxyz" * 100_000
      )
      updateResponse = situation.updateFeature("tenant", featureId, newFeature)
    ...
    "allow to update feature project" in {
      ...
      (originProjectResponse.json.get \ "features")
        .as[JsArray]
        .value mustBe empty
      (targetProjectResponse.json.get \ "features" \ 0 \ "name")
        .as[String] mustEqual "F1"
    }
    ...
    "delete associated local overloads when modifying feature project" in {
      ...
      val secondUpdateResponse = situation.updateFeature(
        "tenant",
        (jsonFeature \ "id").as[String],
        newOldFeature
      )
    ...
    "allow to create new tags while updating feature" in {
      ...
      val testSituation = TestSituationBuilder()
        .loggedInWithAdminRights()
        .withTenants(
          TestTenant(tenantName).withProjects(TestProject(projectName))
        )
        .build()
      val featureResponse =
        testSituation.createFeature(
    ...
    "allow to change RELEASE_DATE date" in {
      ...
      val featureResponse = testSituation.createFeature(
        ...
        conditions = Set(
          TestCondition(period =
            TestDateTimePeriod(begin = LocalDateTime.of(1992, 9, 19, 1, 12, 1))
          )
        )
      )
      val updateResponse = testSituation.updateFeature(
      ...
      updateResponse.status mustBe OK
      (updateResponse.json.get \ "conditions" \\ "begin").head
        .as[String] mustEqual "1993-09-19T01:12:01Z"
    }
    ...
    "allow to change DATE_RANGE dates" in {
      ...
      (json.get \ "conditions" \\ "begin").head
        .as[String] mustEqual "1992-08-19T00:10:00Z"
      (json.get \ "conditions" \\ "end").head
        .as[String] mustEqual "2100-08-19T00:10:00Z"
    }
    ...
    "allow to delete tags" in {
      ...
      updateResponse.status mustBe OK
      (updateResponse.json.get \ "tags" \\ "id").map(v =>
        v.as[String]
      ) mustBe Nil
    }
  }
  ...
  "Feature DELETE endpoint" should {
    ...
    "allow to delete existing feature if user has default write project right" in {
      ...
      val deleteResponse =
        testSituation.deleteFeature(tenantName, featureRequest.id.get)
    ...
    "prevent feature deletion if user has update right on project" in {
      ...
      val situation = TestSituationBuilder()
        .withTenants(
          TestTenant(tenant)
            .withProjects(
              TestProject("my-project").withFeatureNames("my-feature")
            )
        )
        .withUsers(
          TestUser("testu")
      ...
      val featureToDeleteId =
        situation.findFeatureId(tenant, "my-project", "my-feature").get
    ...
    "return 404 for missing feature" in {
      ...
      val deleteResponse = situation.deleteFeature(
        id = "d398cb04-1476-4b32-ae9b-8bb4d5f9f3a5",
        tenant = "foo"
      )
  ...
  "Feature test endpoint" should {
    ...
    "return active if feature is active for given user" in {
      ...
      val res = situation.testFeature(
        ...
        TestFeature(enabled = true, name = "foo")
          .withConditions(
            TestCondition(rule = TestUserListRule(users = Set("aaa")))
          ),
        OffsetDateTime.now(),
        "aaa"
      )
  ...
  "Existing feature test endpoint" should {
    "return correct feature value for number value" in {
      ...
      val checkResponse =
        situation.testExistingFeature("tenant", id, context = "ctx")
      ...
      val updateResponse = situation.updateFeature(
        "tenant",
        id,
        json ++ Json.obj("enabled" -> true)
      )
      updateResponse.status mustBe OK
      val checkResponseEnabled =
        situation.testExistingFeature("tenant", id, context = "ctx")
      checkResponseEnabled.status mustBe OK
      (checkResponseEnabled.json.get \ "active").as[BigDecimal] mustEqual 1.4

      val overloadResult = situation.changeFeatureStrategyForContext(
        "tenant",
        "project",
        "ctx",
        "f1",
                ...
                TestFeatureContext(
                  ...
                  subContext = Set(
                    TestFeatureContext(
                      name = "subcontext",
                      subContext =
                        Set(TestFeatureContext(name = "subsubcontext"))
                    )
                  )
                )
              )
  ...
  "Feature multi test endpoint" should {
    ...
    "return specified features" in {
      ...
      val response = situation.evaluateFeaturesAsLoggedInUser(
        "tenant",
        features =
          Seq(("project", "F1"), ("project", "F3"), ("project2", "F21"))
      )
      response.json.get
        .as[Map[String, JsObject]]
        .values
    ...
    "return correct activation for resulting feature" in {
      ...
      result.json.get
        ...
        .values
        .map(obj =>
          ((obj \ "name").as[String], (obj \ "active").get)
        ) must contain theSameElementsAs Seq(
        ("F1", JsTrue),
        ("F2", JsFalse),
        ("F3", JsTrue),
