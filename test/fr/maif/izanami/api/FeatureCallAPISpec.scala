  ...
  "stale feature tracking" should {
    "not report too old never called feature as stale if stale tracking is disabled" in {
      val situation = TestSituationBuilder()
        ...
        .withCustomConfiguration(
          Map("app.experimental.stale-tracking.enabled" -> "false")
        )
        .loggedInWithAdminRights()
        .build()

      val featureId =
        situation.findFeatureId("test", project = "proj", feature = "feat")
      updateFeatureCreationDateInDB(
        "test",
        featureId.get,
    ...
    "report too old never called feature as stale if stale tracking is disabled" in {
      val situation = TestSituationBuilder()
        ...
        .withCustomConfiguration(
          Map("app.experimental.stale-tracking.enabled" -> "true")
        )
        .loggedInWithAdminRights()
        .build()
