...
import { cleanupConfig, clickAction } from "./utils";
...
test.describe("Global settings page should", () => {
  test("allow to update oidc rights and roles", async ({
    ...
    await page
      .getByRole("option", { name: "Supervised" })
      .click({ force: true });
    await page.getByRole("button", { name: "Update configuration" }).click();
  ...
  test("allow to update max rights", async ({ page, browser }) => {
    ...
    await page.getByRole("button", { name: "Create role" }).click();
    await expect(page.getByText("Rights for role admin")).toBeVisible();

    await page.getByRole("button", { name: "Add tenant right" }).last().click();
    await page
      .getByRole("combobox", { name: "New Tenant" })
      .click({ force: true });
    await page
      .getByRole("option", { name: "secret", exact: true })
      .click({ force: true });
    ...
    await page
      .getByRole("row", { name: "select Sam Tailor Sam Tailor" })
      .getByLabel("actions")
      .click();
    ...
    await expect(page.getByRole("status")).toContainText(
      "User role doesn't allow him to have following rights on tenant secret:\nDefault project right Admin (max allowed is Write)",
    );
    ...
    await expect(page2.locator("#modal")).toMatchAriaSnapshot(`
      ...
      - list:
        - listitem: "Tenant secret: your default project right has been updated from Write to Read"
        - listitem: "Tenant secret: your right on project secretproject has been updated from Write to Read"
      `);
