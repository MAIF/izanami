import { screenshotBuilder, setup } from "./utils.js";
...
async function generate() {
  ...
  await page.getByRole("button", { name: "feature url" }).click();
  ...
  await page.getByRole("button", { name: "Save" }).click();
  const copyKeyLocator = page.getByRole("button", { name: "Copy" });
  await copyKeyLocator.waitFor();
  await page.waitForTimeout(500);
  await screenshot("key-secret");
  await copyKeyLocator.click();
