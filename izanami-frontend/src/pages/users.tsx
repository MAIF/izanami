  ...
  if (userQuery.isLoading) {
            ...
            (tenantName: string) => (
              <div key={`${user.username}-${tenantName}`}>
                <Link to={`/tenants/${tenantName}`}>
                  <button className="btn btn-primary btn-sm"><i className="fas fa-cloud me-2" aria-hidden="true"></i>{`${tenantName}`}</button>
                </Link>
              </div>
            )
          );
    ...
    return (
      <>
        ...
        {creating && (
            <div className="sub_container anim__rightToLeft mb-2" role="form">
