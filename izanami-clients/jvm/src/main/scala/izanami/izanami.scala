...
case class IzanamiDispatcher(name: String = "akka.actor.default-dispatcher",
                             system: ActorSystem) {
...
case class ClientConfig(
  ...
  def withClientSecretHeaderName(clientSecretHeaderName: String) = {
    assert(clientSecretHeaderName != null,
           "clientSecretHeaderName should not be null")
...
object Strategies {
  ...
  @annotation.varargs
  def smartCacheWithPollingStrategy(pollingInterval: FiniteDuration,
                                    patterns: String*) =
...
object Strategy {
  ...
  case class FetchWithCacheStrategy(maxElement: Int, duration: FiniteDuration)
      extends Strategy
...
case class ExperimentFallback(id: String,
                              name: String,
                              description: String,
                              enabled: Boolean,
                              variant: Variant) {
...
case class Experiment(id: String,
                      name: String,
                      description: String,
                      enabled: Boolean,
...
object DefaultFeature {
  ...
  val writes: Writes[DefaultFeature] = Json.writes[DefaultFeature].transform {
    o: JsObject =>
...
case class GlobalScriptFeature(id: String,
                               enabled: Boolean,
...
case class ScriptFeature(id: String,
                         enabled: Boolean,
                         active: Option[Boolean],
                         script: String)
    extends Feature {
...
object ReleaseDateFeature {
  ...
  private val reads: Reads[ReleaseDateFeature] = transform(
    ...
    'date ->> read[LocalDateTime](
      localDateTimeReads(pattern).orElse(localDateTimeReads(pattern2)))
  )
...
case class ReleaseDateFeature(id: String, enabled: Boolean, date: LocalDateTime)
    extends Feature {
...
object FeatureEvent {
  ...
  case class FeatureUpdated(id: String, feature: Feature, oldFeature: Feature)
