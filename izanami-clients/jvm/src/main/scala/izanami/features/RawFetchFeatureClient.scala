...
object RawFetchFeatureClient {
  ...
  ): RawFetchFeatureClient =
    new RawFetchFeatureClient(client, clientConfig, fallback, events)
}

private[features] class RawFetchFeatureClient(
    client: HttpClient,
