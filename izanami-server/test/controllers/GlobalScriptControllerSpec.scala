...
class GlobalScriptControllerSpec(name: String, configurationSpec: Configuration)
  ...
  s"$name GlobalScriptController" should {
    ...
    "create read update delete deleteAll" in {
      val key = "my:path"
      /* First check */
      ws.url(s"$rootPath/api/scripts/$key").get().futureValue.status must be(
        404)
      ...
      val script =
        Json.obj("id" -> key,
                 ...
                 "source" -> "function() {}")
      ws.url(s"$rootPath/api/scripts").post(script).futureValue.status must be(
        201)
      ...
      /* Delete */
      ws.url(s"$rootPath/api/scripts/$key").delete().futureValue.status must be(
        200)
