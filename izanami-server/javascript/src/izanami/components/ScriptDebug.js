...
export class ScriptDebug extends Component {
  ...
  runDebug = () => {
    debugScript(this.props.script, this.props.language, JSON.parse(this.state.context || '{}'))
  ...
  render() {
    return (
      <div>
              ...
              <div className="debug-button-res">
                ...
                {this.state.result &&
                  <div className="debug-result-zone">
                    <div>
                      <span>Result is </span><span className="main-color">{this.state.result} <i className={`main-color fa ${this.state.result ===  'enabled' ? 'fa-thumbs-up': 'fa-thumbs-down'}`}/></span>
                    </div>
                  </div>
                }
            </div>
        ...
        <div className="row">
              <Logs title="Logs" size={this.state.logsExpanded} expand={this.expandLogs} collapse={this.collapseLogs} logs={this.state.logs} />
              <Logs title="Errors" size={this.state.errorsExpanded} expand={this.expandErrors} collapse={this.collapseErrors} logs={this.state.errors} />
        </div>
      </div>
    );
  }
