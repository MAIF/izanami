...
export class ExperimentsExplorerPage extends Component {
  ...
  updateGraph = () => {
    ...
    if (this.state.clientId.trim() === '') {
      this.setState({ graph: { message: 'You have to provide a client id' }, searching: false})
    } else {
      return IzanamiServices.fetchExperimentTree(this.state.query, this.state.clientId).then(graph => {
      ...
      <div className="col-md-12">
        <form className="form-inline" style={{ marginBottom: 20, marginLeft: -15  }}>
          <div className="form-group">
            ...
            <input type="text" className="form-control col-md-10" style={{ width: 300 }} placeholder="client id (ie. john.doe@maif.fr)" value={this.state.clientId} onChange={e => this.setState({ clientId: e.target.value })} />
          </div>
