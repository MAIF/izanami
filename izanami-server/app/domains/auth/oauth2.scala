...
package object auth {
  ...
  type OAuthModule = PlayModule with UserContext with ZLogger with AuthInfoModule
  ...
  object Oauth2Service {
    ...
    def createOrUpdateUserIfNeeded(authConfig: Oauth2Config,
      ...
      if (authConfig.izanamiManagedUser) {
        ...
        UserService.getByIdWithoutPermissions(id).refineOrDie[IzanamiErrors](PartialFunction.empty).flatMap {
      ...
      (authConfig.readProfileFromToken, authConfig.jwtVerifier.filter(_.enabled)) match {
        case (true, Some(algoConfig)) =>
          for {
            ...
            _ <- ZIO(JWT.require(algo).acceptLeeway(10).build().verify(accessToken))
                  .refineOrDie[IzanamiErrors](PartialFunction.empty)
    ...
    def getPublicKey(keyBytes: Array[Byte], algorithm: String): ZIO[OAuthModule, IzanamiErrors, PublicKey] =
      for {
        kf <- ZIO(KeyFactory.getInstance(algorithm))
               ...
               .refineOrDie[IzanamiErrors](PartialFunction.empty)
        keySpec <- ZIO(new X509EncodedKeySpec(keyBytes)).refineOrDie[IzanamiErrors](PartialFunction.empty)
    ...
    def getPrivateKey(keyBytes: Array[Byte], algorithm: String): ZIO[OAuthModule, IzanamiErrors, PrivateKey] =
      for {
        kf <- ZIO(KeyFactory.getInstance(algorithm))
               ...
               .refineOrDie[IzanamiErrors](PartialFunction.empty)
        keySpec <- ZIO(new PKCS8EncodedKeySpec(keyBytes)).refineOrDie[IzanamiErrors](PartialFunction.empty)
