...
class EventsController(env: Env,
  ...
  private implicit val idExtractor = EventIdExtractor[IzanamiEvent](event =>
    Some(s"${event._id}")) //Some(event.key.key))
  ...
  private def events[T <: IzanamiEvent](domains: Seq[String],
                                        patterns: String) = AuthAction { ctx =>
