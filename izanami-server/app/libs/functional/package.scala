...
package object functional {
  ...
  object Implicits {
    ...
    implicit def mergePlayResult[L <: Result, R <: Result](
        value: EitherT[Future, L, R])(
        implicit ec: ExecutionContext
    ): Future[Result] = value.value.map(_.merge)
    implicit def mergeResult[A](value: EitherT[Future, A, A])(
  ...
  object EitherTOps {
    ...
    def liftFEither[E, A](f: Future[Either[E, A]])(
        implicit ec: ExecutionContext): EitherT[Future, E, A] = EitherT(f)

    def mapLeft[E, E1, A](
        onError: E => E1
    )(f: Future[Either[E, A]])(
        implicit ec: ExecutionContext): EitherT[Future, E1, A] =
      EitherT(f.map(_.left.map(onError)))

    def liftFuture[E, A](f: Future[A])(
        implicit ec: ExecutionContext): EitherT[Future, E, A] =
      EitherT.liftT[Future, E, A](f)

    def liftOption[E, A](ifNone: => E)(o: Option[A])(
        implicit ec: ExecutionContext): EitherT[Future, E, A] =
      EitherT.fromOption[Future](o, ifNone)

    def liftFOption[E, A](ifNone: => E)(o: Future[Option[A]])(
        implicit ec: ExecutionContext): EitherT[Future, E, A] = {
    ...
    def lift[E, A](a: A)(implicit ec: ExecutionContext): EitherT[Future, E, A] =
      EitherT.pure[Future, E, A](a)

    def liftBooleanTrue[E](
        ifFalse: => E
    )(fboolean: Boolean)(
        implicit ec: ExecutionContext): EitherT[Future, E, Boolean] =
      fboolean match {
