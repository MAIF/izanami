...
import elastic.es7.api.{Bulk, BulkOpDetail, BulkOpType, Elastic, EsException, GetResponse}
...
import libs.database.Drivers.{DriverLayerContext, Elastic7Driver}
import zio.{Has, ZLayer}

object Elastic7JsonDataStore {

  def live(dbDomainConfig: DbDomainConfig): ZLayer[Elastic7Driver with DriverLayerContext, Throwable, JsonDataStore] =
...
class Elastic7JsonDataStore(elastic: Elastic[JsValue], elasticConfig: ElasticConfig, dbDomainConfig: DbDomainConfig)(
    implicit actorSystem: ActorSystem
  ...
  private val index = elastic.index(esIndex)
  ...
  override def getById(id: Key): Task[Option[JsValue]] = {
    ...
    IO.fromFuture { implicit ec =>
      index
        .get(id.key)
        .map {
          case r @ GetResponse(_, _, _, true, _) =>
            r.as[EsDocument]
              .some
              .map(_.value)
          case _ =>
  ...
  override def deleteAll(query: Query): IO[IzanamiErrors, Unit] =
    IO.fromFuture { implicit ec =>
        findByQuerySource(query)
          .map {
            case (id, _) =>
              Bulk[EsDocument](BulkOpType(delete = Some(BulkOpDetail(None, Some(id)))), None)
          }
  ...
  override def count(q: Query): Task[Long] = {
    ...
    Task.fromFuture(implicit ec => index.search(query).map(s => s.hits.total.value))
  }
