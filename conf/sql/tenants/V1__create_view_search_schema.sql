...
CREATE VIEW search_entities AS
SELECT
    text 'features' AS origin_table,  id::text AS id, name, ts AS searchable_name
FROM
    features
UNION ALL
SELECT
    text 'projects' AS origin_table, id::text as id,  name, ts AS searchable_name
FROM
    projects
UNION ALL
SELECT
    text 'tags' AS origin_table, id::text as id, name, ts AS searchable_name
FROM
    tags
UNION ALL
SELECT
    text 'apikeys' AS origin_table, clientid as id, name, ts AS searchable_name
FROM
    apikeys;
