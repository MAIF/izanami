...
<Note>Max rights are available for Izanami 2.19.0 and above.</Note>
...
Users that didn't log in since a long time may have stale rights (rights that are not up-to-date with max rights configuration).
For these users, Izanami will apply max-rights to their rights on the fly when reading them, displaying a projection of their updated rights.
