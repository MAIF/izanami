...
Sometimes you need specific conditions for feature activation, for instance to open a feature only to some users or to activate feature only after a given date.
...
Summer-sales only start at a given date, we could manually activate it at the desired time, however Izanami allows to perform this kind of stuff automatically.

You'll need to click "Add condition" button on feature form creation.

<img src={addConditionHover} />

A new condition is added to your feature, by default this conditions does not change your fature behaviour, since it's active for all users and all time period.
...
You can now save your feature, as we can see, feature table of the project screen confirms that our feature will only be active for our specified period.
