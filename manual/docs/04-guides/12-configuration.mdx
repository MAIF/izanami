...
|                            | Environnement variable                | Program argument                | Default     |
| -------------------------- | ------------------------------------- | ------------------------------- | ----------- |
| Cluster mode               | IZANAMI_CLUSTER_MODE                  | app.cluster.mode                | strandalone |

Possible cluster mode values are `standalone`, `leader` or `worker`.


#### Worker instances context allowlist & blocklist

When Izanami is started in worker mode, context allowlist and blocklist indicates which contexts can and cannot be requested.

If neither blocklist nor allowlist are set, all contexts can be requested, which is the default behaviour.

Note that when a context is blocklisted or allowlisted, its children are also considered blocklisted / allowlisted. This means that:

* if `prod` is blaklisted, `prod/mobile` is considered blocklisted as well
* likewise, if `prod` is allowlisted, `prod/mobile` will be considered allowlisted as well


Izanami allows to define **both** allowlist and blocklist, in such cases your context must be allowlisted and not blocklisted
to be accessible. For instance if `prod` is allowlisted and `prod/mobile` is blocklisted, `prod` context will be accessible, 
but `prod/mobile` and all its children won't.


<Note>
`Context allowlist` and `Context blocklist` are deprecated, it's now recommanded to use `Context allowlist by tenant` and `Context blocklist by tenant`.
</Note>
...
```json
{
  "mytenant": ["context1", "context2/subcontext"],
  "anothertenant": ["contextfoo"]
}
```

To value these through env variable, surround them with `'`.

```sh
IZANAMI_CLUSTER_CONTEXT_ALLOWLIST_BY_TENANT='{"mytenant": ["prod"]}'
```

|                             | Environnement variable                          | Program argument                             | Default     |
| --------------------------- | ----------------------------------------------- | -------------------------------------------- | ----------- |
| Context allowlist by tenant | IZANAMI_CLUSTER_CONTEXT_ALLOWLIST_BY_TENANT     | app.cluster.context-allowlist-by-tenant      | \{\}        |
| Context blocklist by tenant | IZANAMI_CLUSTER_CONTEXT_BLOCKLIST_BY_TENANT     | app.cluster.context-blocklist-by-tenant      | \{\}        |
| Context allowlist           | IZANAMI_CLUSTER_CONTEXT_ALLOWLIST               | app.cluster.context-allowlist                | null        |
| Context blocklist           | IZANAMI_CLUSTER_CONTEXT_BLOCKLIST               | app.cluster.context-blocklist                | []          |


#### Worker exposition URL


When using leader / worker mode, you must specify on which URL worker are exposed. This is used to display correct URLs
in UI.

Additionally you can specify which worker to use for each contexts, this is usefull when using allowlist & blocklist on contexts.

Fot these two needs, you can specify url of worker associated with each context.

When computing worker url to use for a given context, Izanami will try to find an exact match. If an exact match does not exist
Izanami look for an url associated with a parent context, etc...

If no url is found, url associated with default context if used. If this is not defined exposition url is used (i.e. leader URL).

For instance for below configuration:
...
```json
{
  "mytenant": {
    "prod": "http://prod.com",
    "dev": "http://dev.com",
    "dev/subdev": "http://subdev.com",
    "": "http://default.com",
  }
}
```

* URL `http://prod.com` will be used for `prod` and `prod/*` contexts (i.e. `prod/mobile`, `prod/whatever`, ...)
* URL `http://dev.com` will be used for `dev` context
* URL `http://subdev.com` will be used for `dev/subdev` context and `dev/subdev/*` contexts
* URL `http://default.com` will be used for any other contexts

To pass IZANAMI_WORKER_URL_BY_CONTEXTS_AND_TENANTS through env variable, just wrap it in `'`:

```sh
IZANAMI_WORKER_URL_BY_CONTEXTS_AND_TENANTS='{"mytenant": {"prod": "http://prod.com","dev": "http://dev.com","dev/subdev": "http://subdev.com","": "http://default.com"}}'
```

|                                           | Environnement variable                            | Program argument                   | Default     |
| ------------------------------------------| ------------------------------------------------- | ---------------------------------- | ----------- |
| Worker URL by context (DEPRECATED)        | IZANAMI_WORKER_URL_BY_CONTEXTS                    | worker-url-by-contexts             | \{\}        |
| Worker URL by context (tenant scoped)     | IZANAMI_WORKER_URL_BY_CONTEXTS_AND_TENANTS        | worker-url-by-contexts-and-tenants | \{\}        |
...
| Allow/preven usage of any wasm based feature   | app.feature.allow-wasm          | IZANAMI_FEATURE_ALLOW_WASM          | true            |
