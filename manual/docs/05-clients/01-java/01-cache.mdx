...
Client put features in cache with their activation conditions.

This means that (except for script features) client can **compute feature activation locally**, without the need to perform an http call to remote Izanami.
In this mode, feature activation conditions are **updated periodically in background**.
...
```java
CompletableFuture<Boolean> result = client.checkFeatureActivation(
