...
If fallback on last known strategy is disabled or if it can't be done (for script feature or never requested features), client will use an error strategy to decide what should be returned.
...
CompletableFuture<Boolean> futureActivation = client
  .checkFeatureActivation(newSingleFeatureRequest("<YOUR_FEATURE_ID>")
...
client.checkFeatureActivations(
  newFeatureRequest().withFeatures(
    SpecificFeatureRequest.feature("<ID_1>").withErrorStrategy(defaultValueStrategy(true)),
    SpecificFeatureRequest.feature("<ID_2>").withErrorStrategy(defaultValueStrategy(false))
  )
)
```
