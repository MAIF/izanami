...
class Postgresql(env: Env) {
  ...
  def updateSchema(): Unit = {
    ...
    val flyway     =
      Flyway.configure
        ...
        .schemas("izanami")
        .placeholders(java.util.Map.of("default_admin", "RESERVED_ADMIN_USER", "default_password", HashUtils.bcryptHash(defaultPassword)))
        .load()
    flyway.migrate()
    dataSource.close()
  }
  ...
  def defaultPassword: String = {
    ...
    val password                  = maybeUserProvidedPassword.getOrElse(IdGenerator.token(24))
    if (maybeUserProvidedPassword.isEmpty) {
      logger.warn(
