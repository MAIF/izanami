...
class Datastores(env: Env) {
  ...
  private implicit val ec = env.executionContext
...
class Env(val configuration: Configuration, val environment: Environment, val Ws: WSClient) {
  ...
  lazy val wasmCacheSize: Int = configuration.getOptional[Int]("app.wasm.cache.size").getOrElse(100)
  ...
  lazy val encryptionKey = new SecretKeySpec(
    configuration.get[String]("app.authentication.tokenBodySecret").padTo(16, "0").mkString("").take(16).getBytes,
    "AES"
  )
