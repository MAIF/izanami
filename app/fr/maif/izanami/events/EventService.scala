...
sealed trait UserEventOrigin extends EventOrigin
...
object EventAuthentication {
  ...
  case object RootAuthentication extends EventAuthentication
  ...
  def eventAuthenticationWrites: Writes[EventAuthentication] = {
    ...
    case TokenAuthentication(token) =>
      Json.obj("authentication" -> "TOKEN", "token" -> token)
    case RootAuthentication => Json.obj("authentication" -> "IZANAMI")
  }
}
...
sealed trait SourceFeatureEvent extends SourceIzanamiEvent {
  ...
  val origin: UserEventOrigin
  val authentication: EventAuthentication

  def withProjectId(projectId: String): SourceFeatureEvent
}

case class SourceConfigurationUpdatedEvent(
    user: String,
    origin: EventOrigin,
    authentication: EventAuthentication,
    oldConfiguration: FullIzanamiConfiguration,
    newConfiguration: FullIzanamiConfiguration
) extends SourceIzanamiEvent {
  override val dbEventType: String = "CONFIGURATION_UPDATED"
  override val entityId: String = "CONFIGURATION"
}
...
case class SourceFeatureCreated(
  ...
  override def withProjectId(projectId: String): SourceFeatureEvent =
    copy(projectId = Some(projectId))
}
...
object EventService {
  ...
  implicit val eventFormat: Format[IzanamiEvent] = new Format[IzanamiEvent] {
    ...
    }.fold(JsError("Failed to read event"): JsResult[IzanamiEvent])(evt =>
      JsSuccess(evt)
    )
  }
      ...
      case f: ConditionFeatureEvent => {
        ...
        Feature
          ...
          .map {
            ...
            case Right(json) => {
              f match {
                case FeatureCreated(_, id, _, _, _, _, _, _, _) =>
                  Some(createEventV2(json, user) ++ Json.obj("id" -> id))
                case FeatureUpdated(_, id, _, _, _, _, previous, _, _, _) => {
                  ...
                  val finalJson = maybePrevious
                    ...
                    .getOrElse(json)
                  val finalEvent =
                    updateEventV2(finalJson, user) ++ Json.obj("id" -> id)
                  Some(finalEvent)
                }
              }
            }
...
class EventService(env: Env) {
        ...
        env.postgresql
          .queryOne(
            s"""
               ...
               |)
               |INSERT INTO  ${
                if (global) "izanami.global_events"
                else s""""${channel}".events"""
              } (id, event, event_type, entity_id, emitted_at, origin, authentication, username)
               |SELECT gid.next_id as id, (jsonb_build_object('eventId', gid.next_id) || $$1::jsonb) as event, $$2::${
                if (global) "izanami.GLOBAL_EVENT_TYPES"
  ...
  def killAllSources(excludeIzanamiChannel: Boolean = false): Future[Unit] = {
    Future
      .sequence(
        sourceMap.keys
          .filterNot(name => excludeIzanamiChannel && name == IZANAMI_CHANNEL)
          .map(killSource)
      )
      .map(_ => ())
  }
