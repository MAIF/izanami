...
class FeatureUsageService(env: Env) {
  ...
  def onStart(): Future[Unit] = {
    callAggregationRegisterCancellation =
      env.actorSystem.scheduler.scheduleAtFixedRate(
        ...
        val callByTenants = callAggregator.calls.groupBy { case (range, _) =>
          range.tenant
        };
        Future
          .sequence(callByTenants.map { case (tenant, map) =>
            featureCalls.registerCalls(
              tenant,
              map.readOnlySnapshot().view.mapValues(al => al.get()).toMap,
              rangeDurationInMinutes
            )
          })
          .map(_ => ())
          .map(_ => callAggregator.clearCalls())
      })
  ...
  def determineStaleStatus(
    ...
    Either[IzanamiError, Seq[LightWeightFeatureWithUsageInformation]]
  ] = {
      ...
      .map(either =>
        either.map(lastCallAndCreationDateByFeature => {
          features.map(feature => {
            val lastCallAndCreationDate =
              lastCallAndCreationDateByFeature(feature.id)
            val staleStatus = lastCallAndCreationDate match {
              case _ if !isStatusTrackingActive => None
              case FeatureUsage(Some(lastCall), _, _) if isTooOld(lastCall) =>
                Some(NoCall(since = lastCall))
              case FeatureUsage(None, creationDate, _)
                  if isTooOld(creationDate) =>
                Some(NeverCalled(since = creationDate))
              case FeatureUsage(_, creationDate, values)
                  if values.size == 1 && isTooOld(creationDate) =>
                Some(
                  NoValueChange(
                    since = valueSearchStateDate,
                    value = values.head
                  )
                )
              case _ => None
            }
            LightWeightFeatureWithUsageInformation(
              feature = feature,
  ...
  private def isTooOld(date: Instant): Boolean =
    Duration.between(date, Instant.now()).compareTo(staleDelay) > 0
