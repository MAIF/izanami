...
object Utils {
  ...
  def contextParamsToString(
      plugin: ExtismCurrentPlugin,
      params: LibExtism.ExtismVal*
  ): String = {
    rawBytePtrToString(plugin, params(0).v.i64, params(1).v.i32)
  }

  def contextParamsToJson(
      plugin: ExtismCurrentPlugin,
      params: LibExtism.ExtismVal*
  ): JsValue = {
...
object LogLevel extends Enumeration {
  ...
  val LogLevelTrace, LogLevelDebug, LogLevelInfo, LogLevelWarn, LogLevelError,
      LogLevelCritical, LogLevelMax = Value
}

object Status extends Enumeration {
  type Status = Value

  val StatusOK, StatusNotFound, StatusBadArgument, StatusEmpty,
      StatusCasMismatch, StatusInternalFailure,
...
object Logging {
  ...
  def proxyLog(): HostFunction[EnvUserData] = HFunction.defineFunction(
    ...
    val messageData =
      Utils.rawBytePtrToString(plugin, params(1).v.i64, params(2).v.i64)
    ...
    HFunction
      .defineContextualFunction("proxy_http_call", config) {
        (
          ...
          {
            val context = Json.parse(Utils.contextParamsToString(
              plugin,
              params.toIndexedSeq: _*
            ))
            val url = (context \ "url").asOpt[String].getOrElse(
            ...
            if (allowed) {
              val builder = env.Ws
                ...
                .withHttpHeaders((context \ "headers").asOpt[Map[
                  String,
                  String
                ]].getOrElse(Map.empty).toSeq: _*)
                .withRequestTimeout(
                  Duration(
                    (context \ "request_timeout").asOpt[Long].getOrElse(
                      30000L
                    ), // TODO
                ...
                .withFollowRedirects(
                  (context \ "follow_redirects").asOpt[Boolean].getOrElse(false)
                )
                .withQueryStringParameters(
                  (context \ "query").asOpt[Map[
                    String,
                    String
                  ]].getOrElse(Map.empty).toSeq: _*
                )
              val bodyAsBytes =
                context.select("body_bytes").asOpt[Array[Byte]].map(bytes =>
                  ByteString(bytes)
                )
              val bodyBase64 = context.select("body_base64").asOpt[String].map(
                str => ByteString(str).decodeBase64
              )
              val bodyJson = context.select("body_json").asOpt[JsValue].map(
                str => ByteString(str.stringify)
              )
              val bodyStr = context
                .select("body_str")
                .asOpt[String]
              ...
              val body: Option[ByteString] =
                bodyStr.orElse(bodyJson).orElse(bodyBase64).orElse(bodyAsBytes)
              ...
              plugin.returnString(
                ...
                Json.stringify(
                  Json.obj(
                    ...
                    "body_base64" -> ByteString(
                      s"you cannot access host: ${urlHost}"
                    ).encodeBase64.utf8String
                  )
                )
              )
            }
