...
object Status extends Enumeration {
  ...
  val StatusOK, StatusNotFound, StatusBadArgument, StatusEmpty,
      StatusCasMismatch, StatusInternalFailure, StatusUnimplemented = Value
}
    ...
    HFunction
      .defineContextualFunction("proxy_http_call", config) {
        (
          ...
          {
            val context = Json.parse(
              Utils.contextParamsToString(plugin, params.toIndexedSeq: _*)
            )
            val url = (context \ "url")
              .asOpt[String]
              .getOrElse("https://request.otoroshi.io")
            ...
            val allowed = allowedHosts.isEmpty || allowedHosts.contains(
              "*"
            ) || allowedHosts.exists(h => RegexPool(h).matches(urlHost))
            if (allowed) {
              val builder = env.Ws
                .url(url)
              ...
              val bodyAsBytes = context
                .select("body_bytes")
                .asOpt[Array[Byte]]
                .map(bytes => ByteString(bytes))
              val bodyBase64 = context
                .select("body_base64")
                .asOpt[String]
                .map(str => ByteString(str).decodeBase64)
              val bodyJson = context
                .select("body_json")
                .asOpt[JsValue]
