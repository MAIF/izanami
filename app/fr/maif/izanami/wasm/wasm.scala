...
case class WasmScriptAssociatedFeatures(
    id: String,
    name: String,
    project: String
)

case class WasmConfigWithFeatures(
    wasmConfig: WasmConfig,
    features: Seq[WasmScriptAssociatedFeatures]
)

object WasmConfigWithFeatures {
  implicit val wasmConfigAssociatedFeaturesWrites
      : Writes[WasmScriptAssociatedFeatures] = { feature =>
    Json.obj(
      "name" -> feature.name,
  ...
  implicit val wasmConfigWithFeaturesWrites: Writes[WasmConfigWithFeatures] = {
    wasm =>
      Json.obj(
...
case class WasmConfig(
  ...
  def allowedHosts: Seq[String] =
    if (authorizations.httpAccess) Seq("*") else Seq()
              ...
              .map(source =>
                WasmConfig(
                  ...
                  config = (json \ "config")
                    ...
                    .getOrElse(Map.empty),
                  allowedPaths = (json \ "allowedPaths")
                    .asOpt[Map[String, String]]
                    .getOrElse(Map.empty),
        ...
        vm.callOpa("execute", context).map {
          case Left(err) =>
            throw new RuntimeException(
              s"Failed to execute wasm feature : ${err.toString()}"
            ) // TODO - fix me
          ...
          case Right(rawResult) => {
            if (rawResult.startsWith("{")) {
              ...
              (response \ "active")
                ...
                .flatMap(json => {
                  expectedType
                    .parse(json)
                    .toRight(
                      WasmResultParsingError(
                        expected = expectedType.toDatabaseName,
                        found = json
                      )
                    )
                })
              ...
              Try { Json.parse(rawResult) }.toEither.left
                .map(ex => {
                  env.logger
                    .error(s"Failed to parse wasm result, result is $rawResult")
                  WasmError()
                })
                .flatMap(json => {
                  expectedType
