...
import play.api.libs.json.{JsError, JsNull, JsObject, JsResult, JsSuccess, JsValue, Json, Reads, Writes, __}
...
sealed trait OldFeature {
  ...
  def toFeature(project: String, zone: ZoneId, globalScriptById: Map[String, OldGlobalScript]): Either[String, (CompleteFeature, Option[OldScript])] = {
    this match {
      ...
      ), description=description.getOrElse(""), tags=tags), None))
      case OldReleaseDateFeature(id, name, enabled, description, tags, date, _) => Right((SingleConditionFeature(id = id, name = Option(name).getOrElse(id), enabled = enabled, project = project, condition =
        DateRangeActivationCondition(begin = Option(date.atZone(zone).toInstant), timezone = zone), description = description.getOrElse(""), tags=tags), None))
      case OldHourRangeFeature(id, name, enabled, description, tags, startAt, endAt, _) => Right((SingleConditionFeature(id = id, name = Option(name).getOrElse(id), enabled = enabled, project = project, condition = ZonedHourPeriod(timezone=zone, hourPeriod=HourPeriod(startTime = startAt, endTime = endAt)), description = description.getOrElse(""), tags=tags), None))
      case OldPercentageFeature(id, name, enabled, description, tags, percentage) => Right((SingleConditionFeature(id = id, name = Option(name).getOrElse(id), enabled = enabled, project = project, condition = UserPercentage(percentage = percentage), description = description.getOrElse(""), tags=tags), None))
...
case class OldDefaultFeature(id: String, name: String, enabled: Boolean, description: Option[String], tags: Set[String], parameters: JsValue = JsNull)
  extends OldFeature
...
case class OldGlobalScriptFeature(id: String, name: String, enabled: Boolean, description: Option[String], tags: Set[String], ref: String)
  extends OldFeature

case class OldScriptFeature(id: String, name: String, enabled: Boolean, description: Option[String], tags: Set[String], script: OldScript)
  extends OldFeature
...
case class OldDateRangeFeature(
                             ...
                             name: String,
                             enabled: Boolean,
                             description: Option[String], tags: Set[String],
                             from: LocalDateTime,
                             to: LocalDateTime,
                             timezone: ZoneId = null
                              ) extends OldFeature

case class OldReleaseDateFeature(id: String, name: String, enabled: Boolean, description: Option[String], tags: Set[String], releaseDate: LocalDateTime, timezone: ZoneId = null)
  extends OldFeature
...
case class OldPercentageFeature(id: String, name: String, enabled: Boolean, description: Option[String], tags: Set[String], percentage: Int)
  extends OldFeature
...
object OldFeature {
    ...
    f.condition match {
      case DateRangeActivationCondition(begin, end, timezone) => {
        {for(
          b <- begin;
          e <- end
        ) yield OldDateRangeFeature(id = f.id, name=f.name, enabled = f.enabled, description = Option(f.description), from = b.atZone(timezone).toLocalDateTime, to = e.atZone(timezone).toLocalDateTime, timezone = timezone, tags=f.tags)
        }.orElse(begin.map(b => OldReleaseDateFeature(id = f.id, name=f.name, enabled = f.enabled, description = Option(f.description), releaseDate=b.atZone(timezone).toLocalDateTime, timezone = timezone, tags=f.tags)))
          .getOrElse(throw new RuntimeException("Failed to convert SingleConditionFeature to OldFeature, this should not happen, please file an issue"))
      }
      case ZonedHourPeriod(HourPeriod(start, end), timezone) => OldHourRangeFeature(id=f.id, name=f.name, enabled=f.enabled, description = Option(f.description), startAt = start, endAt = end, timezone=timezone, tags=f.tags)
      case UserPercentage(percentage) => OldPercentageFeature(id=f.id, name=f.name, enabled = f.enabled, description = Some(f.description), percentage = percentage, tags=f.tags)
      case UserList(users) => OldCustomersFeature(id=f.id, name=f.name, enabled = f.enabled, description = Option(f.description), customers = users.toList, tags=f.tags)
      case All => OldDefaultFeature(f.id, f.name, f.enabled, description = Option(f.description), parameters = Json.obj(), tags=f.tags)
    }
  }
  ...
  implicit val hourRangeReads: Reads[OldHourRangeFeature] = (
    ...
    )((id, name, enabled, description, tags, start, end) => OldHourRangeFeature(id, name, enabled, description, tags, start, end))
  ...
  implicit val langageRead: Reads[OldPluginLangage] = json => {
    json.asOpt[String].map(_.toUpperCase).map(l => l match {
  ...
  implicit val dateRangeReads: Reads[OldDateRangeFeature] = (
    ...
    )((id, name, enabled, description, tags, from, to) => OldDateRangeFeature(id=id, name=name, enabled=enabled, description=description, tags=tags, from=from, to=to))

  implicit val releaseDateReads: Reads[OldReleaseDateFeature] = (
    commonRead and
      (__ \ "parameters" \ "releaseDate")
        .read[LocalDateTime](
          localDateTimeReads(dateTimePattern).orElse(localDateTimeReads(dateTimePattern2)).orElse(localDateTimeReads(dateTimePattern3))
        )
    )((id, name, enabled, description, tags, releaseDate) => OldReleaseDateFeature(id=id, name=name, enabled=enabled, description=description, releaseDate=releaseDate, tags=tags))

  val oldFeatureReads: Reads[OldFeature] = {json =>
    (json \ "activationStrategy").asOpt[String].map {
  ...
  val oldFeatureWrites: Writes[OldFeature] = {
    ...
    case f: OldScriptFeature => throw new RuntimeException("Failed to write OldGlobalScriptFeature, this should no happen")
