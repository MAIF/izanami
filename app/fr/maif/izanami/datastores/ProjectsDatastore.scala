...
class ProjectsDatastore(val env: Env) extends Datastore {
  ...
  def createProjects(
      ...
      env.postgresql.queryRaw(
        ...
        .flatMap {
          ...
          case Right(value) => env.postgresql.queryRaw(
  ...
  def readTenantProjectForUser(tenant: String, user: String): Future[List[Project]] = {
    ...
    env.postgresql.queryAll(s"""
      ...
      """, List(user), schemas = Set(tenant)) { row => row.optProject() }
  }

  def deleteProject(tenant: String, project: String): Future[Either[IzanamiError, List[String]]] = {
    env.postgresql.executeInTransaction(conn => {
      env.postgresql
        .queryOne(s"""DELETE FROM projects p WHERE p.name=$$1 RETURNING (SELECT array_agg(f.id) AS ids FROM features f WHERE f.project=p.name);""",
          List(project),
          schemas = Set(tenant),
          conn=Some(conn)
        ...
        .flatMap {
          ...
          case Right(ids) => Future.sequence(ids.map(id => env.datastores.features.emitEvent(tenant, id, FeatureDeleted, sourceProject=project, conn=conn))).map(_ => Right(ids))
        }
    })
