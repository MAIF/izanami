...
class FeaturesDatastore(val env: Env) extends Datastore {
  ...
  def findFeatureMatching(tenant: String, pattern: String, clientId: String, count: Int, page: Int): Future[(Int, Seq[AbstractFeature])] = {
  ...
  def findByRequestForKey(
    ...
    val needContexts = request.context.nonEmpty

    val params = if (needContexts) {
      List(clientId, request.projects.toArray, request.features.toArray, possibleContextPaths.toArray)
    } else {
      List(clientId, request.projects.toArray, request.features.toArray)
    }
    ...
    env.postgresql
      .queryAll(
        s"""
         ...
         |    w.config
         |    ${if (needContexts) {
          """,fcs.enabled as overload_enabled, fcs.conditions as overload_conditions, fcs.context as overload_context,ow.config as overload_config, fcs.context_path"""
        } else {
          """,null as overload_enabled, null as overload_conditions, null as overload_context, null as overload_config, null as context_path"""
        }}
         ...
         |  GROUP BY f.id, pid, w.config, ${if (needContexts)
          "ow.config, fcs.enabled, fcs.conditions, fcs.context, fcs.context_path, "
        else ""}  k.clientsecret
         |) SELECT filtered_features.clientsecret, filtered_features.pid AS project_id,
         ...
         |           ${if (needTags) "'tags', filtered_features.tags," else ""}
         ...
         |GROUP BY filtered_features.pid, filtered_features.clientsecret;
         |""".stripMargin,
        params,
        schemas = Set(tenant)
      ) { r =>
        {
          for (
            p      <- r.optUUID("project_id");
            secret <- r.optString("clientsecret").filter(readed => clientSecret == readed) // TODO put this in above query
          )
            yield (
              secret,
              p,
              r.optJsArray("features")
                ...
                .map(featureDuplicates =>
                  featureDuplicates
                    .sortWith((first, second) => {
                      val firstContext  = (first \ "context").asOpt[String]
                      val secondContext = (second \ "context").asOpt[String]
                ...
                .flatMap(f => {
                  ...
                  Feature.readFeature(js).asOpt.toSeq
                })
                .filter(f => request.features.contains(f.id) || request.allTagsIn.subsetOf(f.tags.map(UUID.fromString)))
                .filter(f =>
                  request.features.contains(f.id) || request.oneTagIn.isEmpty || request.oneTagIn.exists(u =>
                    f.tags.contains(u.toString)
                  )
                )
                .filter(f =>
      ...
      .filter(ls =>
        ls.headOption.forall {
          case (secret, _, _) => secret == clientSecret
        }
      )
      .map(ls => ls.map { case (_, project, features) => (project, features) })
    ...
    env.postgresql.executeInTransaction(
      conn => {
        ...
        (feature match {
          ...
          .flatMap(_ => env.postgresql.queryRaw(
            s"""
               ...
               |""".stripMargin, List(id, feature.project), conn=Some(conn)){_ => Some(())}
          )
          .flatMap(_ =>
          env.postgresql
            .queryOne(
    ...
    env.postgresql.executeInTransaction(conn =>
      env.postgresql
        ...
        .flatMap {
          ...
          case Right((id, project))   => emitEvent(tenant, id = id, eventType = FeatureDeleted, sourceProject=project, conn = conn).map(_ => Right(id))
        }
    )
  }
}
