...
class FeatureContextDatastore(val env: Env) extends Datastore {
  ...
  def createGlobalFeatureContext(
    ...
    env.postgresql.executeInTransaction(conn => {
      env.postgresql
        ...
        .flatMap {
          case Left(value) => Left(value).future
          case Right(ctx) => env.postgresql.queryOne(
            s"""
  ...
  def readStrategyForContext(
    ...
    val possibleContextIds = possibleContextPaths.map(path => {
    ...
    isLocal.flatMap(local =>
      env.postgresql
        .queryOne(
         ...
         |RETURNING feature
         |""".stripMargin,
          List(project, if(local)generateSubContextId(project, path) else generateSubContextId(tenant, path), feature),
          schemas=Set(tenant)
        ) { r => Some(()) }
