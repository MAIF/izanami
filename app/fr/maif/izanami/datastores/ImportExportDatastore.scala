...
import fr.maif.izanami.models.TagType
    ...
    Future
      .sequence(
        entries.toSeq
          ...
          .map(t =>
            tableMetadata(tenant, t._1.table)
              .map(maybeMetdata => (maybeMetdata, t._2, t._1))
          )
      )
      .flatMap(s => {
                          ...
                          case ImportController.Fail =>
                            doImportTenantData(
                              ...
                              if (importResult.failedElements.isEmpty) {
                                ...
                                Left(PartialImportFailure(
                                  Map(t._3 -> importResult.failedElements.toSeq)
                                ))
                              }
                            })
                          case Replace =>
              ...
              .flatMap {
                ...
                case Right(result) => {
                  ...
                  Future
                    ...
                    .flatMap(_ => {
                      println(
                        s"Looking for strategy for feature ${result.createdFeatures ++ result.updatedFeatures} (in connection)"
                      )
                              ...
                              if (result.updatedFeatures.nonEmpty) {
                                val previousFeatureStates: Future[
                                  ...
                                  if (
                                    ...
                                    val ids = entries(FeatureType)
                                      .map(f => (f \ "id").as[String])
                                      .toSet
                                    println(
                                      s"Looking for strategy for feature ${ids} (not in connection)"
                                    )
                                    env.datastores.features
                                      .findActivationStrategiesForFeatures(
                                        tenant,
                  ...
                  .flatMap(_ => {
                    env.postgresql
                      .queryOne(
                        ...
                        env.postgresql.queryRaw("RELEASE SAVEPOINT savepoint") {
                          _ => r
                        }
                      )
...
object ImportExportDatastore {
  ...
  case class DBImportResult(
    ...
    def addFailedElement(json: JsObject): DBImportResult =
      copy(failedElements = failedElements + json)
  ...
  object DBImportResult {
    def from(m: Map[ExportedType, UnitDBImportResult]): DBImportResult = {
      m.foldLeft(DBImportResult())((res, t) => {
        ...
        } else if (t._1 == FeatureTagType) {
          // TODO ensure that features are processed BEFORE feature tags
          val upsertedFeatures = t._2.updatedElements
          val hasBeenCreated = upsertedFeatures.groupBy(featureId =>
            res.createdFeatures.contains(featureId)
          )
          res.withCreatedFeatures(
            res.createdFeatures ++ hasBeenCreated.getOrElse(true, Set())
          ).withUpdatedFeatures(
            res.updatedFeatures ++ hasBeenCreated.getOrElse(false, Set())
          )
        } else {
          res
        }
