...
class UsersDatastore(val env: Env) extends Datastore {
  ...
  // FIXME add right reduction when max rights
  ...
  def findUserWithCompleteRights(
    ...
    // FIXME reduce rights with maxRights if needed
...
object userImplicits {
  ...
  def dbRightToRight(dbRight: String): RightLevel = {
    dbRight match {
      ...
      case _ => throw new IllegalArgumentException(s"Unknown right $dbRight")
    }
  }
