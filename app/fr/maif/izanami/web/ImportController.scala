...
import fr.maif.izanami.web.ExportController.parseExportedType
  ...
  def importV2Data(
    ...
    (for (
      ...
      Try(scala.io.Source.fromFile(uri)).toEither.left
        .map(ex => {
          InternalServerError(Json.obj("message" -> "Failed to read file"))
        })
        .map(bf => {
          bf.getLines()
            .flatMap(line => {
        ...
        .map(_ => Ok(""))
    }).toRight(BadRequest(Json.obj("message" -> "Missing export file"))).flatten.fold(r => r, r => r).future
  }
                ...
                features = maybeEitherFeatures
                  ...
                  .map {
                    case CompleteWasmFeature(
                          ...
                          project,
                          enabled,
                          WasmConfig(scriptName, _, _, _, _, _, _, _, _, _, _, _),
                          tags,
                          metadata,
                          description
