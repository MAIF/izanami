...
import fr.maif.izanami.errors.ImportRowIsNotJsonObject
import fr.maif.izanami.errors.MissingImportRow
import fr.maif.izanami.errors.MissingImportType
import fr.maif.izanami.errors.UnknownImportType
...
class ImportController(
      ...
      implicit request =>
        {
          env.datastores.tenants.readImportStatus(UUID.fromString(id)).map {
            ...
            case None =>
              NotFound(Json.obj("message" -> "Can't find this import"))
          }
        }
    }
  ...
  private def parseImportV2Line(line: String)
      : Either[IzanamiError, (ExportedType, JsObject)] = {

    for (
      rowAsJson <- Try { Json.parse(line) }.toEither.left.map(_ =>
        ImportRowIsNotJsonObject
      );
      dbRow <- (rowAsJson \ "row").asOpt[JsObject].toRight(MissingImportRow);
      rowTypeStr <- (rowAsJson \ "_type").asOpt[String].toRight(
        MissingImportType
      );
      rowType <- parseExportedType(rowTypeStr).toRight(
        UnknownImportType(rowTypeStr)
      )
    ) yield (rowType, dbRow)
  }
  ...
  def importV2Data(
    ...
    (for (
      uri <- files.get("export").toRight(MissingImportFile);
      bf <- Try { scala.io.Source.fromFile(uri) }.toEither.left.map(_ =>
  ...
  def fixImportDataIfNeeded(
    ...
    features = features
      .map {
        case obj if !obj.keys.contains("result_type") => {
          val res: JsObject =
            obj + ("result_type" -> JsString(BooleanResult.toDatabaseName))
          res
        }
        case obj => obj
      }
      .map {
        case obj if !obj.keys.contains("created_at") => {
          val res: JsObject =
            obj + ("created_at" -> JsString(formatter.format(Instant.now())))
          res
        }
        case obj => obj
      }

    if (!featureService.isWasmAllowed) {
      val forbiddenFeatures = data
        .getOrElse(FeatureType, Seq())
        .filter(json => {
          json.keys.contains("script_config") && (json \ "script_config")
            .asOpt[String]
            .isDefined
        })
        .map(json => (json \ "id").asOpt[String])
        .flatMap(o => o.toSet)

      if (forbiddenFeatures.nonEmpty) {
        messages.append(
          s"Some features are wasm dependant features, this Izanami instance is configured to forbid wasm features therefore these features will be considered disabled on evaluation. Please update these features to remove wasm dependencies. Feature ids are: ${forbiddenFeatures.mkString(",")}."
        )
      }

      val forbiddenOverloads = data
        .getOrElse(OverloadType, Seq())
        .filter(json => (json \ "script_config").asOpt[String].isDefined)
        .map(json => {
          for (
            project <- (json \ "project").asOpt[String];
            context <- (json \ "context").asOpt[String];
            feature <- (json \ "feature").asOpt[String]
          ) yield {
            s"${feature} (project $project, context $context)"
          }
        })
        .flatMap(o => o.toSet)

      if (forbiddenOverloads.nonEmpty) {
        messages.append(
          s"Some overloads are wasm dependant, this Izanami instance is configured to forbid wasm features therefore these overloads will be considered disabled on evaluation. Please update these overloads to remove wasm dependencies. Feature ids are: ${forbiddenOverloads.mkString(",")}."
        )
      }
    ...
    overloads =
      overloads.map(json => castOldOverloadToNewOverloadIfNeeded(json))
    ...
    keys = keys.map(obj => {
      ...
      if (!clientId.startsWith(s"${tenant}_")) {
        ...
        messages.append(
          s"""New tenant name is different from old one (it changed from ${parts(
              0
            )} to $tenant).
    ...
    val contexts = data
      .getOrElse(ContextType, Seq())
      .concat(
        data
          .getOrElse(LocalContextType, Seq())
          .map(json => oldContextToNewContext(LocalContextType, json))
      )
      .concat(
        data
          .getOrElse(GlobalContextType, Seq())
  ...
  def castOldOverloadToNewOverloadIfNeeded(json: JsObject): JsObject = {
    val maybeOldContext = (json \ "local_context")
      .asOpt[String]
      .orElse((json \ "global_context").asOpt[String])
  ...
  def oldContextToNewContext(
      contextType: ExportedType,
      json: JsObject
  ): JsObject = {
    if (contextType != GlobalContextType && contextType != LocalContextType) {
      throw new IllegalArgumentException(
        s"Failed to convert old context to new context : unknown type $contextType"
      )
    }
    ...
    val parent = (json \ "parent")
      .asOpt[String]
      .orElse((json \ "global_parent").asOpt[String])
      .map(p => {
    ...
    Json
      ...
      .applyOnWithOpt(maybeProject)((json, proj) =>
        json + ("project" -> JsString(proj))
      )
  }
  ...
  def importV1Data(
    ...
    def runImport(id: UUID): Future[ImportResult] = {
      ...
      val maybeEitherScripts = files
        ...
        .map(uri =>
          unnest(
            readFile(uri, OldFeature.globalScriptReads).map(_.map(Right(_)))
          )
        )
        .map(either =>
          either.map(scripts =>
            scripts.map(s => s.copy(id = scriptIdToNodeCompatibleName(s.id)))
          )
        )
        ...
        .map(uri =>
          unnest(
            readFile(uri, OldFeature.oldFeatureReads).map(features => {
              val globalScriptById: Map[String, OldGlobalScript] =
                maybeEitherScripts
              ...
              features.map(feature => {
                extractProjectAndName(feature, projectChoiceStrategy)
                  .flatMap { case (project, name) =>
                    feature
                      .toFeature(project, ZoneId.of(timezone), globalScriptById)
                      .map { case (feature, maybeScript) =>
                        (feature.withName(name), maybeScript)
                      }
                  }
              })
            })
      ...
      val errors =
        maybeEitherFeatures.map(e => e.swap.getOrElse(Seq())).toSeq.flatten
      ...
      val eitherData =
        if (Seq(maybeEitherFeatures, keys, users).forall(_.isEmpty)) {
          ...
          val projects = if (deduceProject) {
            maybeEitherFeatures
              .flatMap(_.toOption.map(_.map(_._1.project)))
              .getOrElse(Seq())
              .toSet
          } else {
            Set(project.get)
          ...
          val maybeEitherUsers =
            users.map(t =>
              unnest(
                t.map(users =>
                  users.map(u => toNewUser(tenant, u, projects, deduceProject))
                )
              )
            )
          val maybeEitherKeys =
            keys.map(t =>
              unnest(
                t.map(keys =>
                  keys.map(k =>
                    Right(toNewKey(tenant, k, projects, deduceProject))
                  )
                )
              )
            )
          ...
          val errors = Seq
            .apply[Option[Either[Seq[String], Seq[Product with Serializable]]]](
              maybeEitherUsers,
              maybeEitherKeys,
              maybeEitherScripts
            )
          ...
          val (compatibleScripts, incompatibleScript) =
            oldScripts.partition(s => {
          ...
          if (errors.nonEmpty) {
            ...
            Right(
              MigrationData(
                ...
                users =
                  maybeEitherUsers.map(_.getOrElse(Seq())).getOrElse(Seq()),
      ...
      (strategy, eitherData) match {
        case (None, _) =>
          ImportFailure(id, Seq("Unknown conflict handling strategy")).future
        case (_, Left(errors)) => ImportFailure(id, errors).future
        case (
              Some(conflictStrategy),
              Right(
          ...
          env.postgresql.executeInTransaction(conn => {
            scripts
              .foldLeft(
                Future.successful[
                  Either[IzanamiError, Map[String, (String, ByteString)]]
                ](Right(Map()))
              )((f, script) => {
                f.flatMap {
                  case Right(ids) =>
                            ...
                            path =
                              if (isBase64)
                                java.util.Base64.getEncoder.encodeToString(
                                  scriptIds(wasmConfig.name)._2.toArray
                                )
                    ...
                    .flatMap {
                      ...
                      case Right(_) =>
                        env.datastores.users
                          ...
                          .map {
                            case Left(error) =>
                              ImportFailure(id, Seq(error.message))
                            case Right(_) =>
                              ImportSuccess(
                                id = id,
    ...
    strategy match {
      ...
      case DeduceProject(fieldCount) => {
        ...
        if (parts.length <= fieldCount) {
          Left(
            s"Feature ${id} is too short to exclude its first ${fieldCount} parts as project name"
          )
        } else {
