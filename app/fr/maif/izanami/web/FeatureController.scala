...
import fr.maif.izanami.errors.{FeatureNotFound, IzanamiError, TagDoesNotExists}
...
class FeatureController(
  ...
  def testFeature(tenant: String, user: String, date: Instant): Action[JsValue] = authenticatedAction.async(parse.json) {
    implicit request =>
      {
        Feature.readFeature((request.body.as[JsObject]) + ("name" -> Json.toJson("test")), "") match {
          ...
          case JsSuccess(feature, _) => {
            val featureToEval = feature match {
              case w: WasmFeature if w.wasmConfig.source.kind == WasmSourceKind.Local => w.copy(wasmConfig = w.wasmConfig.copy(source = w.wasmConfig.source.copy(path = s"${tenant}/${w.wasmConfig.source.path}")))
              case f => f
            }
            Feature
                ...
                .map(feature =>
                  env.datastores.featureContext
                    ...
                    .flatMap {
                      ...
                      case None           =>
                        Feature
                          .writeFeatureForCheck(feature, RequestContext(tenant = tenant, user = user, now = date, context=context), env)
                          .map {
  ...
  def checkFeatureForContext(
    ...
    authTuple match {
      case Some((clientId, clientSecret)) => {
        ...
        futureTenant.flatMap {
          case Some(tenant) => env.datastores.features.findByIdForKey(tenant, id, context.elements, clientId, clientSecret).map(maybeFeature => maybeFeature.map(feature => (tenant, feature)))
          case None => Future.successful(None)
        }.flatMap {
          case Some((tenant, feature)) => Feature
            .writeFeatureForCheck(feature, RequestContext(tenant = tenant, user = user, context=context), env = env, includeDetails = conditions)
            .map {
              case Left(error) => error.toHttpResponse
              case Right(json) => Ok(json)
            }
        ...
        authTuple match {
          ...
          case Some((clientId, clientSecret)) => {
            val futureMaybeTenant = ApiKey
              .extractTenant(clientId).map(t => Future.successful(Some(t)))
              .getOrElse(env.datastores.apiKeys.findLegacyKeyTenant(clientId))

              futureMaybeTenant.flatMap {
                case None => Forbidden(Json.obj("message" -> "Key is not authorized for this tenant")).future
                case Some(tenant) => {
                  ...
                  futureFeaturesByProject.transformWith {
                    case Failure(exception) => InternalServerError("").future
                    case Success(Left(error)) => error.toHttpResponse.future
                        ...
                        Future
                          .sequence(
                            featuresByProjects.values.flatten
                              .map(feature =>
                                Feature.writeFeatureForCheck(feature, context=RequestContext(tenant = tenant, user = user, now = date.getOrElse(Instant.now()), context=FeatureContextPath(featureRequest.context)), env=env,includeDetails = conditions)
        ...
        Future
          .sequence(
            featuresByProjects.values.flatten
              ...
              .map(feature =>
                feature
                  .active(RequestContext(tenant = tenant, user = user, now = date.getOrElse(Instant.now()), context = FeatureContextPath(featureRequest.context)), env)
                  .map(either => (feature, either))
                  .map {
                    case (feature, Left(error)) => feature.id -> Json.obj("error" -> error.message, "name" -> feature.name, "project" -> feature.project)
                    case (feature, Right(active)) => feature.id -> Json.obj("active" -> active, "name" -> feature.name, "project" -> feature.project)
                  }
              )
          )
          .map(_.toMap)
