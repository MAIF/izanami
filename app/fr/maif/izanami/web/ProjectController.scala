...
class ProjectController(
        ...
        env.datastores.events
          ...
          .map {
            case (events, maybeCount) => {
              ...
              Ok(
                Json.obj("events" -> Json.toJson(events), "count" -> jsonCount)
              )
            }
          }
    }
    ...
    projectAuthAction(tenant, project, ProjectRightLevel.Admin).async(
      ...
      Project.projectReads.reads(request.body) match {
        case JsSuccess(updatedProject, _) =>
          env.datastores.projects
            .updateProject(tenant, project, updatedProject, request.user)
            .map {
  ...
  def readProjects(tenant: String): Action[AnyContent] =
    personnalAccessTokenDetailledRightForTenantFactory(tenant, ReadTenant)
      ...
      env.datastores.projects
        ...
        .flatMap(maybeProject => {
          maybeProject.fold(
            ...
            project => {
              featureUsageService
                .determineStaleStatus(tenant, project.features)
                .map {
                  case Left(err)                           => err.toHttpResponse
                  case Right(featuresWithUsageInformation) => {
                    val projectWithUsageInformation =
                      ProjectWithUsageInformation
                        .fromProject(
                          project = project,
                    ...
                    Ok(
                      Json.toJson(projectWithUsageInformation)(
