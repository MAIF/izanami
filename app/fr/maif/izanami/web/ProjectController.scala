...
class ProjectController(
        ...
        env.datastores.events
          .listEventsForProject(
            ...
            FeatureEventRequest(
              ...
              eventTypes = parseStringSet(types).map(t =>
                EventService.parseEventType(t)
                  ...
                  (
                    events.map(e => {
                      val json = Json.toJson(e)(
                        EventService.eventFormat.writes(_)
                      ).as[JsObject]
                      e.authentication match {
                        case EventAuthentication.TokenAuthentication(
                              tokenId
                            ) => {
                        ...
                        case EventAuthentication.BackOfficeAuthentication =>
                          json
  ...
  def createProject(tenant: String): Action[JsValue] =
  ...
  def readProjects(tenant: String): Action[AnyContent] =
    personnalAccessTokenDetailledRightForTenantFactory(
      tenant,
      ReadTenant
    ).async {
      implicit request =>
        val isTenantAdmin = request.user.tenantRight.exists(right =>
  ...
  def readProject(tenant: String, project: String): Action[AnyContent] =
    personnalAccessTokenAuthAction(
      tenant,
      project,
      ...
      env.datastores.projects
        ...
        .flatMap(maybeProject => {
          maybeProject.fold(
            ...
            project => {
              featureUsageService.determineStaleStatus(
                tenant,
                project.features
              ).map {
                ...
                case Right(featuresWithUsageInformation) => {
                  val projectWithUsageInformation = ProjectWithUsageInformation
                    .fromProject(
                      project = project,
                      features = featuresWithUsageInformation.toList
                    )
                  Ok(Json.toJson(projectWithUsageInformation)(
                    projectWithUsageInformationWrites
                  ))
                }
              }
            }
