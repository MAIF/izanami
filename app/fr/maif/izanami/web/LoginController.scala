                            ...
                            env.datastores.users
                              ...
                              .flatMap(maybeUser => {
                                def createOrUpdateUser() = {
                                  rightService
                                    .generateRightsForRoles(roles)
                                    .map {
                                      case Some(rightForRole) => rightForRole
                                    ...
                                    }).recover(ex => Left(InternalServerError))
                                }
                                createOrUpdateUser().flatMap {
                                  case Left(value)  =>
                                    env.datastores.configuration
                                      .updateOIDCRightByRolesIfNeeded(userRightsByRoles)
                                      .flatMap(_ => createOrUpdateUser())
