...
class ConfigurationController(
  ...
  def readExpositionUrl(): Action[AnyContent] = Action { implicit request =>
    env.configuration.getOptional[String]("app.exposition.backend")
      .orElse(env.configuration.getOptional[String]("app.exposition.url")) match {
      case Some(value) => Ok(Json.obj("url" -> value))
      case None => NotFound
    }
  }
