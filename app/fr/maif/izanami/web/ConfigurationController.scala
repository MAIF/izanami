...
import fr.maif.izanami.models.IzanamiConfiguration.{
  ...
  mailGunConfigurationWrite,
  mailJetConfigurationWrites,
  mailerReads
}
import fr.maif.izanami.services.{MaxRights, MaxTenantRoleRights}
...
class ConfigurationController(
  ...
  private def updateOIDCRightIfNeeded(
      conf: FullIzanamiConfiguration
  ): Future[FullIzanamiConfiguration] = {
    conf.oidcConfiguration
      .flatMap(_.userRightsByRoles)
      .fold(conf.future)(rights => {
        env.datastores.configuration
          .updateOAuthRightByRole(rights)
          .map(updatedRights =>
            conf
              .copy(oidcConfiguration =
                conf.oidcConfiguration.get
                  .copy(userRightsByRoles = Some(rights))
                  .some
              )
          )
      })
