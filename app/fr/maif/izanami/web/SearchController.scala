...
class SearchController(
          ...
          checkSearchParams(query, filter).flatMap {
            ...
            case Right(_)    =>
              ...
              Future
                ...
                .map(l =>
                  l.flatten.toList
                    .sortBy(_._3)(Ordering.Double.TotalOrdering.reverse)
                    .take(10)
                )
                .flatMap(results => {
    ...
    def createContextPath(
      ...
      contexts match {
        ...
        case (g: GlobalContext) :: tail =>
          Seq(GlobalContextPathElement(g.name))
            .concat(createContextPath(tail, hasProject))
        case (g: LocalContext) :: tail =>
          Seq(LocalContextPathElement(g.name))
            .concat(createContextPath(tail, hasProject))
        case _ => Seq()
      }
    }
    ...
    rowType match {
      ...
      case "global_context" | "local_context" => {
        ...
        (rowJson \ "parent")
          ...
          .getOrElse(
            hasProject
              .map(project => Seq(ProjectPathElement(project)))
              .getOrElse(Seq())
              .future
          )
      }
  ...
  private def generateParentCandidates(
      parentParts: Seq[String]
  ): Set[String] = {
    @tailrec
    def act(
        current: Seq[String],
        res: Seq[String],
        previousItems: Seq[String]
    ): Seq[String] = {
      val newRes =
        if (previousItems.nonEmpty) res.appended(previousItems.mkString("_"))
        else res
      if (current.isEmpty) {
        newRes
      } else {
