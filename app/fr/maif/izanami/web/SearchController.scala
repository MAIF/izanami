...
import fr.maif.izanami.models.Context
import fr.maif.izanami.models.GlobalContext
import fr.maif.izanami.models.LocalContext
...
class SearchController(
                    ...
                    .map(tenant =>
                      env.datastores.search
                        .tenantSearch(
                          ...
                          query,
                          filter.map(item =>
                            SearchEntityObject.parseSearchEntityType(item)
                          )
                        )
                        .map(l =>
                          l.map(t => {
            ...
            .flatMap(results => {
              Future.sequence(results.map { case (rowType, rowJson, _) =>
                buildPath(rowType, rowJson, tenant)
                  .map(pathElements => {
                    val jsonPath =
                      Json.toJson(pathElements)(Writes.seq(pathElementWrite))
  ...
  private def checkSearchParams(
    ...
    if (
      filter.nonEmpty && !filter.forall(
        SearchEntityObject.parseSearchEntityType(_).isDefined
      )
    ) {
  ...
  private def buildPath(
      ...
      rowJson: JsObject,
      tenant: String
  ): Future[Seq[PathElement]] = {
    def createContextPath(
        contexts: List[Context],
        hasProject: Option[String]
    ): Seq[PathElement] = {
      contexts match {
        case (g: GlobalContext) :: (l: LocalContext) :: tail => {
          Seq(
            GlobalContextPathElement(g.name),
            ProjectPathElement(l.project),
            LocalContextPathElement(l.name)
          ).concat(createContextPath(tail, hasProject))
        }
        case (g: GlobalContext) :: Nil if hasProject.isDefined => {
          Seq(
            GlobalContextPathElement(g.name),
            ProjectPathElement(hasProject.get)
          )
        }
        case (g: GlobalContext) :: tail =>
          Seq(GlobalContextPathElement(g.name)).concat(createContextPath(
            tail,
            hasProject
          ))
        case (g: LocalContext) :: tail =>
          Seq(LocalContextPathElement(g.name)).concat(createContextPath(
            tail,
            hasProject
          ))
        case _ => Seq()
    ...
    rowType match {
      case "feature" =>
        Seq(ProjectPathElement((rowJson \ "project").as[String])).future
      case "global_context" | "local_context" => {
        ...
        }).getOrElse(hasProject.map(project =>
          Seq(ProjectPathElement(project))
        ).getOrElse(Seq()).future)
      }
