                  ...
                  Result(
                    ...
                    body = HttpEntity.Streamed(
                      org.apache.pekko.stream.scaladsl.Source
                        .single(ByteString(jsons.mkString("\n"), "UTF-8")),
                      None,
...
object ExportController {
  val exportRequestReads: Reads[ExportRequest] = json => {
    (json \ "tenants")
      .asOpt[Map[String, TenantExportRequest]](
  ...
  val tenantExportRequestReads: Reads[TenantExportRequest] = json => {
    (for (
      ...
      yield JsSuccess(
        TenantExportRequest(projects, keys, webhooks, userRights)
      )).getOrElse(JsError("Bad body format"))
  }
  ...
  val tenantExportResultWrites: Writes[TenantExportResult] = exportResult => {
    Json.obj(
      "projects" -> Json.toJson(exportResult.projects)(
  ...
  val projectExportResultWrites: Writes[ProjectExportResult] = exportResult => {
    Json.obj(
      "features" -> Json.toJson(exportResult.features)(
