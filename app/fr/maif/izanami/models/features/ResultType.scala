...
import play.api.libs.json.{JsBoolean, JsError, JsNull, JsNumber, JsString, JsSuccess, JsValue, Reads, Writes}
        ...
        .map {
          case StringResult  => {
            (for (
              ...
              yield StringResultDescriptor(
                value = value,
                conditions = conditions
              )).map(r => JsSuccess(r)).getOrElse(JsError("Failed to read StringResultDescriptor"))
          }
          case NumberResult  => {
            (for (
              ...
              yield NumberResultDescriptor(
                value = value,
                conditions = conditions
              )).map(r => JsSuccess(r)).getOrElse(JsError("Failed to read NumberResultDescriptor"))
          }
