    ...
    (for (
      ...
      (mailer, originEmail) match {
        case (MailerTypes.Console, _) =>
          JsSuccess(IzanamiConfiguration(mailer = mailer, invitationMode = invitationMode, originEmail = originEmail))
        case (_, None)                => JsError("Origin email is missing")
        case (_, maybeEmail)          =>
          JsSuccess(IzanamiConfiguration(mailer = mailer, invitationMode = invitationMode, originEmail = maybeEmail))
      }
