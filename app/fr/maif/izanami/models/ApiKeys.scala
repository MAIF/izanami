...
case class ApiKeyWithCompleteRights(
    tenant: String,
    clientId: String = null,
    clientSecret: String = null,
    name: String,
    projects: Set[ApiKeyProject] = Set(),
    enabled: Boolean,
    legacy: Boolean,
    admin: Boolean
)
...
object ApiKey {
  def read(json: JsValue, tenant: String): JsResult[ApiKey] = (
    (__ \ "clientId").readNullable[String] and
      (__ \ "name").read[String](Reads.pattern(
        NAME_REGEXP,
        s"Name does not match regexp ${NAME_REGEXP.pattern}"
      )) and
      (__ \ "description").readNullable[String] and
      (__ \ "enabled").read[Boolean] and
      (__ \ "projects").readWithDefault[Set[String]](Set())(Reads.set(
        Reads.pattern(
          NAME_REGEXP,
          s"Name does not match regexp ${NAME_REGEXP.pattern}"
        )
  ...
  )((clientId, name, description, enabled, projects, admin) =>
    ApiKey(
