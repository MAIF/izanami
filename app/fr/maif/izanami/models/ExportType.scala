...
object ExportedType {
  val exportedTypeToExportedNameAssociation: Seq[(String, ExportedType)] = Seq(
    ...
    (
      "local_context",
      LocalContextType
    ), // Legacy, for compat reasons with old export format
    (
      "global_context",
      GlobalContextType
    ), // Legacy, for compat reasons with old export format
  ...
  def parseExportedType(typestr: String): Option[ExportedType] = {
    exportedTypeToExportedNameAssociation.find(t => t._1 == typestr).map(t =>
      t._2
    )
  }

  def exportedTypeToString(exportedType: ExportedType): Option[String] = {
    exportedTypeToExportedNameAssociation.find(t => t._2 == exportedType).map(
      t => t._1
    )
  }
}
