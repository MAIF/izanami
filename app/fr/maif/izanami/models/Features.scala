...
object FeaturePatch {
  ...
  implicit val patchPathReads = Reads[PatchPath] { json =>
    json
      .asOpt[String]
  ...
  implicit val patchOpReads = Reads[PatchOperation] { json =>
    json
      .asOpt[String]
      .map {
  ...
  implicit val featurePatchReads = Reads[FeaturePatch] { json =>
    val maybeResult = for (
...
object WasmFeature {
  ...
  val format                                 = new Format[WasmFeature] {
...
object Feature {
  ...
  def writeFeatureForCheck(feature: AbstractFeature, context: RequestContext, env: Env, includeDetails: Boolean = false): Future[Either[IzanamiError, JsValue]] = {
    feature
      .active(context, env)
      .map(either => {
        either.map(active => {
  ...
  implicit val offsetTimeWrites = Writes[OffsetTime] { time =>
    Json.toJson(time.format(DateTimeFormatter.ISO_OFFSET_TIME))
  }

  implicit val offsetTimeReads = Reads[OffsetTime] { json =>
    try {
...
val hourFormatter = DateTimeFormatter.ofPattern("HH:mm:ss")
  implicit val hourPeriodWrites = Writes[HourPeriod] { p =>
    Json.obj(
  ...
  implicit val hourPeriodReads = Reads[HourPeriod] { json =>
    (for (
  ...
  implicit val dayOfWeekWrites = Writes[DayOfWeek] { d =>
    Json.toJson(d.name)
  }

  implicit val dayOfWeekReads = Reads[DayOfWeek] { json =>
    json.asOpt[String].map(DayOfWeek.valueOf).map(JsSuccess(_)).getOrElse(JsError(s"Incorrect day of week : ${json}"))
  }

  implicit val activationDayOfWeekWrites = Writes[ActivationDayOfWeeks] { a =>
    Json.obj(
      "days"     -> a.days
    )
  }

  implicit val activationDayOfWeekReads = Reads[ActivationDayOfWeeks] { json =>
    (for (
  ...
  implicit val featurePeriodeWrite = Writes[FeaturePeriod] { period =>
    if (period.empty) {
      JsNull
    } else {
      Json.obj(
  ...
  implicit val activationConditionWrite = Writes[ActivationCondition] { cond =>
    Json.obj(
      "period" -> cond.period,
      "rule"   -> cond.rule
    )
  ...
  val featureWrite = Writes[AbstractFeature] {
