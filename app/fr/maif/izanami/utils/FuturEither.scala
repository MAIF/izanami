...
case class FutureEither[+A](value: Future[Either[IzanamiError, A]]) {
  ...
  def flatMapF[B](transformer: A => Future[B])(implicit ec: ExecutionContext): FutureEither[B] = {
  ...
  def toResult(mapper: A => play.api.mvc.Result)(implicit ec: ExecutionContext): Future[Result] = {
    value.map {
  ...
  def fold[R](errMapper: IzanamiError => R, valueMapper: A => R)(implicit ec: ExecutionContext): Future[R] = {
    value.map({
...
object FutureEither {
  def from[A](value: Either[IzanamiError, A])(implicit ec: ExecutionContext): FutureEither[A] = FutureEither(Future.successful(value))
  def success[A](value: A): FutureEither[A]            = FutureEither(Future.successful(Right(value)))
  def success[A](value: Future[A])(implicit ec: ExecutionContext): FutureEither[A] = FutureEither(value.map(a => Right[IzanamiError, A](a)))
