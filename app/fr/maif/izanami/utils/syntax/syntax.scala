...
object implicits {
  implicit class BetterSyntax[A](private val obj: A) extends AnyVal {
    ...
    def applyOnIf(predicate: => Boolean)(f: A => A): A =
      if (predicate) f(obj) else obj
    def applyOnWithOpt[B](opt: => Option[B])(f: (A, B) => A): A =
      if (opt.isDefined) f(obj, opt.get) else obj
    def applyOnWithPredicate(predicate: A => Boolean)(f: A => A): A =
      if (predicate(obj)) f(obj) else obj
  ...
  implicit class BetterListEither[E, V](private val obj: Iterable[Either[E, V]])
      extends AnyVal {
  ...
  implicit class BetterFutureEither[V](private val obj: Future[Either[
    IzanamiError,
    V
  ]]) extends AnyVal {
  ...
  implicit class BetterEither[V](private val obj: Either[IzanamiError, V])
      extends AnyVal {
    def toFEither: FutureEither[V] = {
  ...
  implicit class BetterJsValue(private val obj: JsValue) extends AnyVal {
    ...
    def vertxJsValue: Object =
      io.vertx.core.json.Json.decodeValue(Json.stringify(obj))
  }

  implicit class BetterByteString(private val obj: ByteString) extends AnyVal {
    def chunks(size: Int): Source[ByteString, NotUsed] =
      Source(obj.grouped(size).toList)

    def sha256: String = Hex.encodeHexString(
      MessageDigest.getInstance("SHA-256").digest(obj.toArray)
    )

    def sha512: String = Hex.encodeHexString(
      MessageDigest.getInstance("SHA-512").digest(obj.toArray)
    )
  }
  ...
  implicit class BetterString(private val obj: String) extends AnyVal {
    ...
    def sha512: String = Hex.encodeHexString(
      MessageDigest.getInstance(
        "SHA-512"
      ).digest(obj.getBytes(StandardCharsets.UTF_8))
    )

    def sha256: String = Hex.encodeHexString(MessageDigest.getInstance(
      "SHA-256"
    ).digest(obj.getBytes(StandardCharsets.UTF_8)))
  }
}
