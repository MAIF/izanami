...
object implicits {
  ...
  implicit class BetterFutureEither[V](
      private val obj: Future[Either[IzanamiError, V]]
  ) extends AnyVal {
  ...
  implicit class BetterByteString(private val obj: ByteString) extends AnyVal {
    def chunks(size: Int): Source[ByteString, NotUsed] = Source(
      obj.grouped(size).toList
    )
